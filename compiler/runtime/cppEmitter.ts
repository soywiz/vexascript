...
const nativeTypedArrayNames = new Set([
  "Int8Array", "Uint8Array", "Uint8ClampedArray", "Int16Array", "Uint16Array",
  "Int32Array", "Uint32Array", "Float16Array", "Float32Array", "Float64Array",
  "BigInt64Array", "BigUint64Array",
]);

const nativeIntlObjectNames = new Set([
  "Collator", "DateTimeFormat", "DisplayNames", "ListFormat", "Locale",
  "NumberFormat", "PluralRules", "RelativeTimeFormat", "Segmenter",
]);

function nativeIntlObjectKind(name: string): string | null {
  return nativeIntlObjectNames.has(name) ? name : null;
}

function nativeTypedArrayCppType(name: string): string | null {
  return nativeTypedArrayNames.has(name) ? `vexa::Native${name}Object*` : null;
}
...
let activeEnumCppTypes: ReadonlyMap<string, "std::int32_t" | "std::u16string" | "vexa::Value"> = new Map();
let activeEnumStatements: ReadonlyMap<string, EnumStatement> = new Map();
...
type CppVariableDelegateKind = "function" | "objectValue";
interface CppVariableDelegateInfo {
  backingName: string;
  kind: CppVariableDelegateKind;
  valueType: string;
}
let activeVariableDelegates: ReadonlyMap<string, CppVariableDelegateInfo> = new Map();
...
let activeInstanceofTemporaryExpressions: Map<string, string> = new Map();
...
let activeNativeCollectionRepresentation: NativeCollectionRepresentation = "specialized";
...
function identifierName(expression: Expr): string | null {
  return expression instanceof Identifier ? (expression as Identifier).name : null;
}
...
function resolvedDefaultArgument(name: string): Expr | null {
  ...
  while (current instanceof Identifier) {
...
function usesPooledFunctionTypeof(unary: UnaryExpression): boolean {
  ...
  const member = unary.argument instanceof MemberExpression
...
function emitIdentifier(identifier: Identifier): string {
  ...
  if (staticClassName) {
    const staticClass = activeClassStatements.get(staticClassName);
    const staticField = staticClass?.members.find((candidate): candidate is ClassFieldMember =>
      candidate instanceof ClassFieldMember && candidate.isStatic === true && candidate.name.name === identifier.name);
    return `${cppName(staticClassName)}::${staticField ? staticFieldAccessorName(staticField) : cppName(identifier.name)}${staticField ? "()" : ""}`;
  }
  const variableDelegate = activeVariableDelegates.get(identifier.name);
  if (variableDelegate) {
    return variableDelegate.kind === "function"
      ? `${variableDelegate.backingName}()`
      : `vexa::recordGet<${variableDelegate.valueType}>(${variableDelegate.backingName}, u"value")`;
  }
  if (activeImplicitReceiverIdentifiers.has(identifier as Node)) {
    ...
    const delegatedReceiver = currentClass
      ? classDelegateReceiverForMember(currentClass, identifier.name)
      : null;
    if (delegatedReceiver) {
      return `${delegatedReceiver}->${interfacePropertyGetterName(identifier.name)}()`;
    }
    return `${activeThisExpression}->${cppName(identifier.name)}`;
  }
...
function emitTopLevelFunctionValue(statement: FunctionStatement): string {
  ...
  for (let index = 0; index < statement.parameters.length; index += 1) {
    ...
    forwardedArguments.push(emitNativeConversion(
      `std::forward<decltype(${argumentName})>(${argumentName})`,
      targetType
    ));
  }
  const argumentsText = forwardedArguments.join(", ");
  let expectedResultType: string | null = activeExpectedLambdaResultCppType;
  if (!expectedResultType && activeExpectedExpressionCppType) {
    expectedResultType = managedArrayElementType(activeExpectedExpressionCppType);
  }
  if (!expectedResultType && statement.returnType) {
    expectedResultType = cppTypeForDeclaredName(statement.returnType.name);
  }
  const call = `${cppName(statement.name.name)}(${argumentsText})`;
  const result = expectedResultType ? emitNativeConversion(call, expectedResultType) : call;
...
function maybeAutoAwait(expression: Expr, emitted: string): string {
  const call = expression instanceof CallExpression ? expression as CallExpression : null;
...
function emitAsyncResultValue(expression: Expr, resultType: string): string {
  ...
  if (expressionType instanceof NamedType && expressionType.name === "Promise") {
    return emitNativeConversion(`(co_await ${emitWithoutAutoAwait(expression)})`, resultType);
  }
...
function memberParts(expression: Expr): MemberParts | null {
  if (!(expression instanceof MemberExpression)) return null;
  const member = expression as MemberExpression;
  if (member.computed || !(member.property instanceof Identifier)) return null;
...
function intlConstructorName(expression: Expr): string | null {
  if (!(expression instanceof MemberExpression)) return null;
  const member = expression as MemberExpression;
  if (member.computed || identifierName(member.object) !== "Intl") return null;
  const name = identifierName(member.property);
  return name && (nativeIntlObjectNames.has(name) || name === "DurationFormat") ? name : null;
}
...
function pooledStringText(value: string): string {
  const name = activeStringLiteralNames.get(value);
  if (!name) throw new CppEmitError(`Missing pooled C++ string literal '${value}'`);
  return `${name}->value()`;
}
...
function enumCppType(name: string): "std::int32_t" | "std::u16string" | "vexa::Value" {
  return activeEnumCppTypes.get(name) ?? "std::int32_t";
}

function cppTypeForWeakAnalysisKey(type: AnalysisType): string | null {
  return type instanceof BuiltinType && type.name === "object"
...
function nativeCollectionPhysicalCppType(name: NativeCollectionName, semanticTypes: readonly string[]): string {
  if (activeNativeCollectionRepresentation === "generic") {
    if (name === "Map") return "vexa::MapObject<vexa::Value, vexa::Value>*";
    if (name === "Set") return "vexa::SetObject<vexa::Value>*";
    if (name === "WeakMap") return `vexa::WeakMapObject<${nativeWeakObjectKeyCppType}, vexa::Value>*`;
    return `vexa::WeakSetObject<${nativeWeakObjectKeyCppType}>*`;
  }
  return name === "Map" || name === "WeakMap"
    ? `vexa::${name}Object<${semanticTypes[0]}, ${semanticTypes[1]}>*`
    : `vexa::${name}Object<${semanticTypes[0]}>*`;
}

function cppTypeForAnalysisType(type: AnalysisType): string | null {
  if (type instanceof BuiltinType) return cppTypeForBuiltin(type.name);
  if (type instanceof NamedType && activeCppTypeParameters.has(type.name) && !(type.typeArguments?.length)) {
  ...
  if (type instanceof ObjectType) return "vexa::RecordObject*";
  ...
  if (
    ...
    const elementType = cppArrayElementType(namedTypeArgument(type, 0) ?? builtinType("unknown"));
    return elementType ? `vexa::ArrayObject<${elementType}>*` : null;
  }
  if (type instanceof NamedType && type.name === "Promise") {
    const resultType = cppTypeForAnalysisType(namedTypeArgument(type, 0) ?? builtinType("unknown"));
  ...
  if (type instanceof NamedType && (type.name === "Map" || type.name === "ReadonlyMap")) {
    ...
    return nativeCollectionPhysicalCppType("Map", [keyType, valueType]);
  }
  if (type instanceof NamedType && (type.name === "Set" || type.name === "ReadonlySet")) {
    const elementType = cppTypeForAnalysisType(namedTypeArgument(type, 0) ?? builtinType("any")) ?? "vexa::Value";
    return nativeCollectionPhysicalCppType("Set", [elementType]);
  }
  if (type instanceof NamedType && type.name === "WeakMap") {
    ...
    const valueType = cppTypeForAnalysisType(namedTypeArgument(type, 1) ?? builtinType("any")) ?? "vexa::Value";
    return keyType?.endsWith("*") ? nativeCollectionPhysicalCppType("WeakMap", [keyType, valueType]) : null;
  }
  if (type instanceof NamedType && type.name === "WeakSet") {
    ...
    return valueType?.endsWith("*") ? nativeCollectionPhysicalCppType("WeakSet", [valueType]) : null;
  }
  ...
  if (type instanceof NamedType) {
    const typedArrayType = nativeTypedArrayCppType(type.name);
    if (typedArrayType) return typedArrayType;
  }
  ...
  if (type instanceof NamedType && type.name === "Segments") return "vexa::IntlSegmentsObject*";
  if (type instanceof NamedType && nativeIntlObjectKind(type.name)) return "vexa::IntlObject*";
...
function computeCppTypeForDeclaredName(typeName: string, visitedAliases: Set<string>): string {
  ...
  if (activeEnumNames.has(typeName)) return enumCppType(typeName);
  ...
  if (shape.baseName === "Set" || shape.baseName === "ReadonlySet") {
    const elementType = cppTypeForDeclaredNameOr(shape.typeArguments[0] ?? "any", "vexa::Value", visitedAliases);
    return nativeCollectionPhysicalCppType("Set", [elementType]);
  }
  if (shape.baseName === "WeakMap") {
    const keyType = cppTypeForWeakDeclaredKey(shape.typeArguments[0] ?? "unknown");
    const valueType = cppTypeForDeclaredNameOr(shape.typeArguments[1] ?? "any", "vexa::Value", visitedAliases);
    return keyType?.endsWith("*") ? nativeCollectionPhysicalCppType("WeakMap", [keyType, valueType]) : "";
  }
  if (shape.baseName === "WeakSet") {
    const valueType = cppTypeForWeakDeclaredKey(shape.typeArguments[0] ?? "unknown");
    return valueType?.endsWith("*") ? nativeCollectionPhysicalCppType("WeakSet", [valueType]) : "";
  }
  ...
  const typedArrayType = nativeTypedArrayCppType(shape.baseName);
  if (typedArrayType) return typedArrayType;
  ...
  if (shape.baseName === "Segments" || shape.baseName === "Intl.Segments") return "vexa::IntlSegmentsObject*";
  if (nativeIntlObjectKind(shape.baseName) ||
      (shape.baseName.startsWith("Intl.") && nativeIntlObjectKind(shape.baseName.slice("Intl.".length)))) {
    return "vexa::IntlObject*";
  }
...
function cppArrayElementType(type: AnalysisType): string | null {
  if (type instanceof BuiltinType && type.name === "string") return "std::u16string";
  if (type instanceof LiteralType && type.base === "string") return "std::u16string";
  if (type instanceof UnionType) return "vexa::Value";
...
function declaredTypeNameForIdentifier(name: string): string | undefined {
  const localTypeName = activeLocalDeclaredTypeNames.get(name);
  if (localTypeName) return localTypeName;
  const globalTypeName = activeGlobalDeclaredTypeNames.get(name);
  if (globalTypeName) return globalTypeName;
  return undefined;
}

function storedCppTypeForIdentifier(name: string): string | undefined {
  const localCppType = activeLocalCppTypes.get(name);
  if (localCppType) return localCppType;
  const globalCppType = activeGlobalCppTypes.get(name);
  if (globalCppType) return globalCppType;
  return undefined;
}
...
function computeCppTypeForExpression(expression: Expr): string {
  ...
  if (!activeHasExpressionTypes && expression instanceof BinaryExpression) {
  ...
  if (expression instanceof SatisfiesExpression || expression instanceof NonNullExpression) {
    return cppTypeForExpression((expression as SatisfiesExpression | NonNullExpression).expression);
  }
  if (expression instanceof ArrayLiteral) {
    const elementType = arrayLiteralCppElementType(expression as ArrayLiteral);
    if (elementType === "double") return `vexa::ArrayObject<${elementType}>*`;
  }
  ...
  if (expression instanceof MemberExpression) {
    ...
    if (!member.computed && member.property instanceof Identifier &&
      member.object instanceof Identifier && (member.object as Identifier).name === "this") {
  ...
  if (expression instanceof NewExpression) {
    ...
    if (constructorMember?.objectName === "Intl" && nativeIntlObjectNames.has(constructorMember.propertyName)) {
      return "vexa::IntlObject*";
    }
    ...
    if (name) {
      const typedArrayType = nativeTypedArrayCppType(name);
      if (typedArrayType) return typedArrayType;
    }
  ...
  if (expression instanceof BooleanLiteral) return "bool";
  if (expression instanceof StringLiteral || expression instanceof NullLiteral ||
      expression instanceof UndefinedLiteral) return "vexa::Value";
  if (expression instanceof ArrayLiteral) {
...
function managedArrayCppTypeForExpression(expression: Expr): string | null {
  ...
  if (type instanceof ArrayType || type instanceof TupleType) return cppTypeForAnalysisType(type);
  return null;
}

function isManagedArrayExpression(expression: Expr): boolean {
  if (expression instanceof Identifier && activeGcArrayTypes.has((expression as Identifier).name)) return true;
  ...
  return type instanceof ArrayType || type instanceof TupleType ||
    (type instanceof NamedType && isNativeArrayTypeName(type.name)) ||
...
function isArrayExpression(expression: Expr): boolean {
  ...
  if (managedArrayElementType(emittedCppTypeForExpression(expression) ?? "") !== null) return true;
  const type = activeExpressionTypes.get(expression as Node);
  return type instanceof ArrayType || type instanceof TupleType || type instanceof RangeType ||
    (type instanceof NamedType && isNativeArrayTypeName(type.name)) ||
    expression instanceof ArrayLiteral || expression instanceof RangeExpression;
}
...
function nativeCollectionKind(expression: Expr): "map" | "set" | "weakMap" | "weakSet" | null {
  ...
  if (expression instanceof CallExpression) {
    ...
    if (getMember?.propertyName === "get") {
      ...
      if (nestedType?.startsWith("vexa::MapObject<")) return "map";
      if (nestedType?.startsWith("vexa::SetObject<")) return "set";
      if (nestedType?.startsWith("vexa::WeakMapObject<")) return "weakMap";
      if (nestedType?.startsWith("vexa::WeakSetObject<")) return "weakSet";
    }
  }
...
function nativeCollectionPointerCppType(expression: Expr): string | null {
  ...
  if (type) {
    ...
    if (type instanceof UnionType || type instanceof IntersectionType) candidates = type.types;
    for (const candidate of candidates) {
      if (!(candidate instanceof NamedType)) continue;
...
function isDateExpression(expression: Expr): boolean {
  const type = activeExpressionTypes.get(expression as Node);
  return type instanceof NamedType && type.name === "Date";
}

function isStringExpression(expression: Expr): boolean {
  const type = activeExpressionTypes.get(expression as Node);
  const localCppType = expression instanceof Identifier
    ? activeLocalCppTypes.get((expression as Identifier).name)
    : null;
  const interfaceProperty: InterfacePropertyMember | null = expression instanceof MemberExpression && !(expression as MemberExpression).computed &&
    (expression as MemberExpression).property instanceof Identifier
...
function nativeCollectionArguments(call: CallExpression | NewExpression): Expr[] {
  return call instanceof CallExpression
...
function nativeCollectionCppType(
  ...
  if (typeArguments.length) {
    ...
    if (explicit.length !== expectedCount) {
      throw new CppEmitError(`C++ ${name} expects ${expectedCount} explicit type argument${expectedCount === 1 ? "" : "s"}`, call);
    }
    mapped = nativeCollectionPhysicalCppType(name, explicit);
  }
  if (activeExpectedExpressionCppType &&
  ...
  if (explicit.length === 0 && !activeExpectedExpressionCppType && argumentsList.length === 1) {
    ...
    if (name === "Map" && collectionType?.startsWith("vexa::MapObject<")) {
      ...
      const elementType = collectionType?.startsWith("vexa::SetObject<")
        ? cppTemplateArguments(collectionType, "vexa::SetObject<")?.[0]
        : iterable instanceof ArrayLiteral
          ? arrayLiteralCppElementType(iterable)
          : managedArrayElementType(managedArrayCppTypeForExpression(iterable) ?? "");
      if (elementType && elementType !== "auto") mapped = `vexa::SetObject<${elementType}>*`;
    }
  }
  ...
  if (!mapped.endsWith("*") && name === "WeakMap") {
    mapped = `vexa::WeakMapObject<${nativeWeakObjectKeyCppType}, vexa::Value>*`;
  }
  if (!mapped.endsWith("*") && name === "WeakSet" && argumentsList.length === 1) {
    ...
    const analysisElementType = arrayAnalysisType instanceof ArrayType
      ...
      : arrayAnalysisType instanceof NamedType &&
        ...
        : null;
    const inferredElementType = elementType === "vexa::Value" &&
        analysisElementType instanceof BuiltinType && analysisElementType.name === "object"
      ? nativeWeakObjectKeyCppType
      : elementType;
  ...
  if (activeNativeCollectionRepresentation === "generic") {
    mapped = nativeCollectionPhysicalCppType(name, explicit);
    explicit = cppTemplateArguments(mapped, `vexa::${name}Object<`) ?? explicit;
  }
...
function emitNativeCollectionConstruction(call: CallExpression | NewExpression, name: NativeCollectionName): string {
  ...
  if (argumentsList.length === 1) {
    ...
    const mappedTypes = cppTemplateArguments(mapped, `vexa::${name}Object<`) ?? [];
    if (name === "Map") {
      if (mappedTypes.length !== 2) throw new CppEmitError("C++ cannot infer Map entry types", call);
      return `vexa::mapFromIterable<${mappedTypes[0]}, ${mappedTypes[1]}>(${emitExpression(values)})`;
    }
    if (name === "Set") {
      if (mappedTypes.length !== 1) throw new CppEmitError("C++ cannot infer Set element type", call);
      return `vexa::setFromIterable<${mappedTypes[0]}>(${emitExpression(values)})`;
    }
    if (name === "WeakMap") {
      if (mappedTypes.length !== 2 || !mappedTypes[0]!.endsWith("*")) {
        throw new CppEmitError("C++ cannot infer WeakMap entry types", call);
      }
      return `vexa::weakMapFromIterable<${mappedTypes[0]}, ${mappedTypes[1]}>(${emitExpression(values)})`;
    }
    if (!isManagedArrayExpression(values) || mappedTypes.length !== 1 || !mappedTypes[0]!.endsWith("*")) {
      throw new CppEmitError("C++ WeakSet iterable construction requires a typed native object array", call);
    }
    return `vexa::weakSetFromArray<${mappedTypes[0]}>(${emitManagedArrayPointer(values)})`;
  }
  return `vexa::makeManaged<${mapped.slice(0, -1)}>()`;
}

function isRecordExpression(expression: Expr): boolean {
  return activeExpressionTypes.get(expression as Node) instanceof ObjectType || expression instanceof ObjectLiteral;
}

function isGeneratorExpression(expression: Expr): boolean {
  const emittedType = expression instanceof Identifier
    ? activeLocalCppTypes.get((expression as Identifier).name)
    : null;
  if (emittedType?.startsWith("vexa::Generator<") || emittedType?.startsWith("vexa::AsyncGenerator<")) return true;
  const type = activeExpressionTypes.get(expression as Node);
  if (type instanceof NamedType && (type.name === "Generator" || type.name === "AsyncGenerator")) return true;
  ...
  if (!method || !(method instanceof ClassMethodMember)) return false;
...
function directlyEmittedMemberCppType(member: MemberExpression): string | null {
  ...
  if (binaryKind &&
      (propertyName === "byteLength" || propertyName === "byteOffset" ||
        (binaryKind === "typedArray" && propertyName === "length"))) {
    return "double";
  }
...
function emitSpecializedNativeConversion(emitted: string, resultType: string): string | null {
  switch (resultType) {
    case "std::u16string":
      return `vexa::toText(${emitted})`;
    case "bool":
      return `vexa::toBoolean(${emitted})`;
    case "double":
      return `vexa::toDouble(${emitted})`;
    case "std::int32_t":
      return `vexa::toNativeInt32(${emitted})`;
    case "vexa::BigInt":
      return `vexa::toBigInt(${emitted})`;
    case "vexa::Undefined":
      return `vexa::toUndefined(${emitted})`;
    case "vexa::Null":
      return `vexa::toNull(${emitted})`;
    case "vexa::Error":
      return `vexa::toError(${emitted})`;
    default:
      return resultType.startsWith("std::function<")
        ? `vexa::toFunction<${resultType}>(${emitted})`
        : null;
  }
}

function emitNativeConversion(emitted: string, resultType: string): string {
  if (resultType === "vexa::Value") return `vexa::toValue(${emitted})`;
  if (resultType.endsWith("*")) return `vexa::toInstance<${resultType}>(${emitted})`;
  return emitSpecializedNativeConversion(emitted, resultType) ??
    `vexa::convertValue<${resultType}>(${emitted})`;
}
...
function emitConvertedValue(expression: Expr, resultType: string): string {
  ...
  if (resultType === "vexa::Value" && expression instanceof StringLiteral) {
  ...
  const alreadyDynamicRecordMember = expression instanceof MemberExpression &&
    activeExpressionTypes.get((expression as MemberExpression).object as Node) instanceof ObjectType;
  if (resultType === "vexa::Value" &&
      (sourceType instanceof FunctionType || expression instanceof ArrowFunctionExpression || expression instanceof FunctionExpression) &&
      !alreadyDynamicRecordMember) {
    const callableExpression = expression instanceof ArrowFunctionExpression || expression instanceof FunctionExpression
    ...
    const result = `vexa::Value(vexa::makeFunction<${templateArguments}>(${emitExpression(expression)}, {${roots.join(", ")}}))`;
  ...
  if (resultType.endsWith("*")) {
    ...
    if (interfaceName) {
      ...
      return `vexa::adaptInterface<${resultType.slice(0, -1)}, ${recordInterfaceAdapterName(interfaceName)}>(${emitExpression(expression)})`;
    }
  }
  const emitted = emitExpression(expression);
  const specialized = emitSpecializedNativeConversion(emitted, resultType);
  if (specialized) return specialized;
  if (resultType.endsWith("*") &&
      (expression instanceof NullLiteral || expression instanceof UndefinedLiteral)) {
    return `${emitted}.toInstance<${resultType}>()`;
  }
  return emitNativeConversion(emitted, resultType);
}
...
function emitArrayElements(elements: readonly Expr[], elementType: string): string {
  ...
  if (hasExpandedElements) {
    const operations = elements.map((element) => {
      ...
      if (element instanceof SpreadExpression) {
        ...
        return elementType === "vexa::Value"
          ? `vexa::appendAllConverted(__vexa_array, ${source})`
    ...
    return `([&]() { auto* __vexa_array = vexa::makeArray<${elementType}>(); ${operations.join("; ")}; return __vexa_array; }())`;
  }
  ...
  return `vexa::makeArray<${elementType}>({${emittedElements.join(", ")}})`;
}
...
function arrayLiteralElementCppType(element: Expr): string {
  ...
  if (element instanceof NullLiteral || element instanceof UndefinedLiteral) return "vexa::Value";
  if (element instanceof SpreadExpression) {
...
function arrayElementCanUseCppType(element: Expr, expectedElementType: string): boolean {
  ...
  if (element instanceof NullLiteral || element instanceof UndefinedLiteral) {
...
function objectPropertyName(property: ObjectProperty): string | null {
  ...
  if (property.key instanceof Identifier) return (property.key as Identifier).name;
...
function objectLiteralPropertyValue(expression: Expr, propertyName: string): Expr | null {
  if (!(expression instanceof ObjectLiteral)) return null;
  const property = (expression as ObjectLiteral).properties.find((candidate) =>
    candidate instanceof ObjectProperty && objectPropertyName(candidate as ObjectProperty) === propertyName);
  return property instanceof ObjectProperty ? (property as ObjectProperty).value : null;
}
...
function emitObjectLiteral(object: ObjectLiteral): string {
  ...
  const emitPropertyValue = (name: string, value: Expr): string => {
    ...
    return expectedType
      ? `vexa::toValue(${emitExpressionWithExpectedCppType(value, expectedType)})`
  ...
  if (simple) {
    ...
    result = `vexa::makeRecord({${properties.join(", ")}})`;
  } else {
    const operations = object.properties.map((property) => {
      ...
      const key = objectProperty.computed
        ...
        : cppUtf16String(name!);
      return `vexa::recordSet(__vexa_record, ${key}, ${emitPropertyValue(name ?? "", objectProperty.value)})`;
    });
    result = `([&]() { auto* __vexa_record = vexa::makeRecord(); ${operations.join("; ")}; return __vexa_record; }())`;
  }
...
function callableParameterName(parameter: CallableParameter): string | null {
  return parameter.name instanceof Identifier ? (parameter.name as Identifier).name : null;
}
...
function builtinCallCppType(expression: Expr): string {
  if (!(expression instanceof CallExpression)) return "";
...
function callableExpressionResultCppType(
  ...
  expression.parameters.forEach((parameter, index) => {
    if (!(parameter.name instanceof Identifier)) return;
...
function emitNativePointerExpression(expression: Expr, expectedPointerType: string | null = null): string {
  ...
  return `vexa::toInstance<${targetType}>(${emitted})`;
}
...
function emitNativeReceiverCall(
  ...
  return `vexa::optionalCall(${receiver}, [&](auto* __vexa_optional_receiver) { return ${emitCallWithReceiver("__vexa_optional_receiver")}; })`;
}
...
function emitDynamicCallArgument(argument: Expr): string {
  const value = argument instanceof NamedArgument ? (argument as NamedArgument).value : argument;
  if (!(value instanceof ArrowFunctionExpression) && !(value instanceof FunctionExpression)) {
...
function computeEmittedCppTypeForExpression(expression: Expr): string | null {
  ...
  if (expression instanceof UnaryExpression && (expression as UnaryExpression).operator === "await") {
  ...
  if (expression instanceof CallExpression) {
    ...
    if (callMember?.propertyName === "resolvedOptions" &&
        (emittedCppTypeForExpression(callMember.object) === "vexa::IntlObject*" ||
         cppTypeForExpression(callMember.object) === "vexa::IntlObject*")) {
      return "vexa::RecordObject*";
    }
    if (callMember?.propertyName === "segment" &&
        (emittedCppTypeForExpression(callMember.object) === "vexa::IntlObject*" ||
         cppTypeForExpression(callMember.object) === "vexa::IntlObject*")) {
      return "vexa::IntlSegmentsObject*";
    }
    if (callMember?.propertyName === "containing" &&
        (emittedCppTypeForExpression(callMember.object) === "vexa::IntlSegmentsObject*" ||
         cppTypeForExpression(callMember.object) === "vexa::IntlSegmentsObject*")) {
      return "vexa::Value";
    }
  ...
  if (expression instanceof BinaryExpression && (expression as BinaryExpression).operator === "??") {
    const binary = expression as BinaryExpression;
    const indexedArrayType = binary.left instanceof MemberExpression &&
    ...
    if (binary.right instanceof NewExpression || binary.right instanceof CallExpression) {
      const fallbackCall = binary.right as NewExpression | CallExpression;
      const fallbackName = identifierName(fallbackCall.callee);
      if (
        fallbackName &&
        new Set<string>(["Map", "Set", "WeakMap", "WeakSet"]).has(fallbackName) &&
        declaredLeft.startsWith(`vexa::${fallbackName}Object<`)
      ) {
        return declaredLeft;
      }
    }
  ...
  if (expression instanceof NewExpression || expression instanceof CallExpression) {
    ...
    if (expression instanceof CallExpression) {
      ...
      if (member?.objectName === "Promise" && new Set(["all", "race", "any"]).has(member.propertyName)) {
        ...
        if (taskArray instanceof ArrayLiteral) {
          for (const element of (taskArray as ArrayLiteral).elements) {
            if (element instanceof SpreadExpression) continue;
      ...
      if (member?.objectName && nativeTypedArrayNames.has(member.objectName) &&
          (member.propertyName === "of" || member.propertyName === "from")) {
        return nativeTypedArrayCppType(member.objectName);
      }
      ...
      if (member && new Set(["then", "catch", "finally"]).has(member.propertyName)) {
        ...
        if (receiverType.startsWith("vexa::Task<")) {
          if (member.propertyName === "then") {
            const callback = callExpression.args[0];
            if (callback && (callback instanceof ArrowFunctionExpression || callback instanceof FunctionExpression)) {
      ...
      if (member && isArrayExpression(member.object)) {
        ...
        if (member.propertyName === "reduce" || member.propertyName === "reduceRight") {
          const semanticResult = cppTypeForExpression(callExpression);
          if (semanticResult !== "auto") return semanticResult;
  ...
  if (expression instanceof MemberExpression) {
    ...
    if (!member.computed && generatorResultType && member.property instanceof Identifier) {
    ...
    if (!member.computed && identifierName(member.property) === "length" &&
      (isManagedArrayExpression(member.object) || isStringExpression(member.object) ||
        nativeBinaryObjectKind(member.object) === "typedArray")) {
...
function emitArguments(
  ...
  const emitArgument = (argument: Expr, index: number): string => {
    ...
    return parameterCppType && argument instanceof ConditionalExpression
      ? emitConvertedValue(argument, parameterCppType)
      : parameterCppType && /^(?:vexa::)?(?:Map|Set|WeakMap|WeakSet)Object</.test(parameterCppType) &&
          (argument instanceof NewExpression || argument instanceof CallExpression)
      ...
      : parameterCppType && managedArrayElementType(parameterCppType) !== null &&
          (argument instanceof ArrayLiteral ||
      ...
      : parameterIsInterface && parameterCppType !== argumentCppType
        ? canStaticallyConvertClassToInterface(argument, parameterCppType!)
          ? `static_cast<${parameterCppType}>(${emitExpression(argument)})`
          : emitConvertedValue(argument, parameterCppType!)
      : argument instanceof UndefinedLiteral && parameterCppType?.endsWith("*")
        ? "nullptr"
  ...
  if (restIndex >= 0) {
    ...
    const restArguments = ordered.slice(restIndex).filter((argument) =>
      !(argument instanceof ArrayLiteral && (argument as ArrayLiteral).__vexaEmptyRest));
...
function emitAnalyzedCallArguments(call: CallExpression): string {
  ...
  if (!(callable instanceof FunctionType)) return emitCallArguments(call);
...
function emitExpressionWithExpectedCallableType(expression: Expr, expectedCppType: string): string {
  ...
  if (!match || (!(expression instanceof ArrowFunctionExpression) && !(expression instanceof FunctionExpression))) {
...
function classNameForExpression(expression: Expr): string | null {
  ...
  const type = activeExpressionTypes.get(expression as Node);
  return type instanceof NamedType && isNativeObjectTypeName(type.name)
...
function declaredTypeNameForExpression(expression: Expr): string | null {
  ...
  if (!(member.property instanceof Identifier)) return null;
  const receiverTypeName = declaredTypeNameForExpression(member.object);
  if (!receiverTypeName) return null;
  const receiverShape = parseTypeNameShape(receiverTypeName);
  ...
  if (statement instanceof ClassStatement) {
    const field = statement.members.find((candidate): candidate is ClassFieldMember =>
      candidate instanceof ClassFieldMember && candidate.name.name === (member.property as Identifier).name);
...
function hasValueBackedClassProperty(expression: Expr): boolean {
  ...
  if (expression instanceof AsExpression ||
      expression instanceof SatisfiesExpression ||
...
function canStaticallyConvertClassToInterface(expression: Expr, resultType: string): boolean {
  const sourceType = emittedCppTypeForExpression(expression) ?? cppTypeForExpression(expression);
  const sourceClass = classStatementForCppType(sourceType);
  if (!sourceClass || interfaceStatementForCppType(resultType) === null) return false;
  return implementedInterfaceTypes(sourceClass).some((implementedType) =>
    cppTypeForDeclaredName(implementedType.name) === resultType);
}
...
function declaredCallResultType(call: CallExpression): string | null {
  ...
  if (!(call.callee instanceof MemberExpression)) return null;
...
function cppTypeForBoundDeclaredName(typeName: string, bindings: ReadonlyMap<string, string>): string | null {
  ...
  if (isNativeMapTypeName(shape.baseName)) {
    return nativeCollectionPhysicalCppType("Map", [argumentsList[0]!, argumentsList[1]!]);
  }
  if (isNativeSetTypeName(shape.baseName)) {
    return nativeCollectionPhysicalCppType("Set", [argumentsList[0]!]);
  }
...
function inferredFunctionCallCppType(call: CallExpression): string | null {
  if (!(call.callee instanceof Identifier)) return null;
...
function staticClassNameForExpression(expression: Expr): string | null {
  if (!(expression instanceof Identifier)) return null;
  const name = (expression as Identifier).name;
...
function accessorValueCppType(
  getter: ClassMethodMember | null,
  setter: ClassMethodMember | null,
  ...
  if (setter && setter.parameters.length > 0) {
    const parameter = setter.parameters[0]!;
    if (parameter.typeAnnotation) {
      const mapped = cppTypeForDeclaredName(parameter.typeAnnotation.name);
      if (mapped.length > 0) return mapped;
    }
  }
  return null;
...
function interfaceMethodForName(statement: InterfaceStatement, methodName: string): InterfaceMethodMember | null {
  ...
  return member instanceof InterfaceMethodMember ? member : null;
}
...
function interfacePropertyForName(statement: InterfaceStatement, propertyName: string): InterfacePropertyMember | null {
  ...
  return member instanceof InterfacePropertyMember ? member : null;
}
...
function dynamicStructuralCastSource(expression: Expr): Expr | null {
  if (!(expression instanceof AsExpression) || cppTypeForExpression(expression) !== "vexa::RecordObject*") {
    return null;
  }
...
function resolvedNativePropertyMember(expression: Expr): NativePropertyMember | null {
  ...
  if (expression instanceof Identifier && activeImplicitReceiverIdentifiers.has(expression as Node)) {
    ...
    const valueType = accessorValueCppType(getter, setter, currentClassPropertyCppType(propertyName));
  ...
  if (activeExpressionTypes.get(member.object as Node) instanceof ObjectType ||
  ...
  const storedProperty = classStoredPropertyInfo(statement, propertyName);
  if (storedProperty) storedType = storedProperty.valueType;
  const valueType = accessorValueCppType(getter, setter, storedType);
  return getter || setter
    ? new NativePropertyMember(
        "method",
        expression,
...
function emitNativePropertyGet(
  ...
  if (property.kind === "record") {
    ...
    return `vexa::recordGet<${resultType === "auto" ? "vexa::Value" : resultType}>(${receiver}, ${key ?? emitNativePropertyKey(property)})`;
  }
...
function emitNativePropertySet(
  ...
  if (property.kind === "record") {
    return `vexa::recordSet(${receiver}, ${key ?? emitNativePropertyKey(property)}, ${value})`;
  }
  if (property.kind === "dynamic") {
    const helper = property.keyExpression ? "dynamicIndexSet" : "dynamicSet";
    return `vexa::${helper}(${receiver}, ${key ?? emitNativePropertyKey(property)}, vexa::toValue(${value}))`;
  }
  if (property.kind === "extension") {
    const converted = property.valueType && property.valueType !== "vexa::Value"
      ? emitNativeConversion(value, property.valueType)
      : value;
    ...
    if (builtinCallCppType(expression) === expectedCppType) {
      ...
      result = actual === expectedCppType
        ...
        ["vexa::MapObject<", "vexa::SetObject<", "vexa::WeakMapObject<", "vexa::WeakSetObject<"]
          .some((prefix) => expectedCppType.startsWith(prefix) && expectedCppType.endsWith(">*"))) {
      result = emitted;
  ...
  if (assignment.operator === "=") {
    ...
    const assignedValue = expectedType
      ...
      : (property.kind === "dynamic" || property.kind === "record") &&
          (activeExpressionTypes.get(assignment.right as Node) instanceof FunctionType ||
            assignment.right instanceof ArrowFunctionExpression ||
            assignment.right instanceof FunctionExpression)
...
function classMethodForMember(
  ...
  if (mappedClass) {
    ...
    if (method) return method;
    for (const implementedType of implementedInterfaceTypes(mappedClass)) {
      const interfaceStatement = activeInterfaceStatements.get(parseTypeNameShape(implementedType.name).baseName);
      const delegatedMethod = interfaceStatement
        ? interfaceMethods(interfaceStatement).find((candidate) => candidate.name.name === member.propertyName)
        : undefined;
      if (delegatedMethod) return delegatedMethod;
    }
  }
  ...
  if (staticClassName) {
    ...
    const resolvedClassName = classNameForExpression(member.object);
    if (!resolvedClassName) return null;
...
function emitBoundMethodValue(
  ...
  const callArguments = parameters.map((parameter, index) => {
    ...
    return emitNativeConversion(`__vexa_bound_argument_${index}`, parameterType);
  });
...
function classMethodForName(
  ...
  while (current && !visited.has(current.name.name)) {
    ...
    const own = current.members.find((candidate): candidate is ClassMethodMember =>
      candidate instanceof ClassMethodMember && candidate.name.name === methodName);
    if (own) {
...
function nativeLambdaCapture(
  ...
  if (!activeFunctionObjectCapture && referenceEntryLocals) {
...
function nativeFunctionCaptureNames(expression: Expr): ReadonlySet<string> {
  if (!(expression instanceof ArrowFunctionExpression) && !(expression instanceof FunctionExpression)) return new Set();
  ...
  for (const parameter of callable.parameters) {
    if ((parameter.name as Node) instanceof Identifier) {
  ...
  while (pending.length > 0) {
    ...
    if (node instanceof VarStatement) {
      ...
      let variableName: BindingName = variable.name;
      const firstDeclaration = variable.declarations?.[0];
      if (firstDeclaration) variableName = firstDeclaration.name;
      for (const rawName of bindingIdentifiers(variableName)) {
        const name = rawName as Identifier;
        declared.add(name.name);
      }
    }
...
function nestedClosureCaptureNames(root: Node): ReadonlySet<string> {
  ...
  while (pending.length > 0) {
    const node = pending.pop()!;
    if (node instanceof ArrowFunctionExpression || node instanceof FunctionExpression) {
...
function nativeLambdaRootValues(captureNames: ReadonlySet<string>): string[] {
  ...
  for (const sourceName of activeGcObjectTypes.keys()) {
    if (captureNames.has(sourceName) && !activeGcArrayTypes.has(sourceName)) {
      roots.push(`vexa::toValue(vexa::rawPointer(${cppName(sourceName)}))`);
    }
  }
  for (const sourceName of activeGcArrayTypes.keys()) {
    if (captureNames.has(sourceName)) {
      roots.push(`vexa::toValue(vexa::arrayPointer(${cppName(sourceName)}))`);
    }
  }
  for (const sourceName of activeDynamicValueNames) {
    if (captureNames.has(sourceName)) {
      roots.push(`vexa::toValue(${cppName(sourceName)})`);
    }
  }
  for (const [sourceName, type] of activeLocalCppTypes) {
    if (!type.endsWith("*") || activeGcObjectTypes.has(sourceName) || activeGcArrayTypes.has(sourceName)) continue;
...
function emitNativeLambda(
  ...
  const receiverParameterName = receiverInfo
    ...
    : null;
  const receiverParameter = receiverParameterName !== null
  ...
  const effectiveParameters = receiverParameter !== null
  ...
  let expectedResultType: string | null = expectedLambdaResult;
  if (!expectedResultType && expectedLambdaExpression) {
    expectedResultType = managedArrayElementType(expectedLambdaExpression);
  }
  const previousLocalNames = activeLocalNames;
  const previousLocalCppTypes = activeLocalCppTypes;
  ...
  const implicitReceiverAlias = receiverInfo?.implicitReceiverAlias === true &&
    parametersList[0]?.name instanceof Identifier
    ? (parametersList[0]!.name as Identifier).name
    : null;
  if (receiverInfo !== undefined && receiverParameterName !== null) {
    ...
    if (receiverCppType.endsWith("*") && receiverInfo.receiverType instanceof NamedType) {
      activeGcObjectTypes.set(labeledReceiverName, receiverInfo.receiverType.name);
    }
    if (receiverInfo.receiverType instanceof NamedType) {
  ...
  try {
    ...
    const receiverPreamble = labeledReceiverName !== null && receiverParameterName !== null
      ? [`  auto ${labeledReceiverName} = ${receiverParameterName};`]
      : [];
    if (implicitReceiverAlias !== null && receiverParameterName !== null) {
    ...
    const rawBody = body instanceof BlockStatement
      ? emitBlock(
...
function emitAsyncNativeLambda(
  ...
  let contextualReturn: string | null = activeExpectedLambdaResultCppType;
  if (!contextualReturn && activeExpectedExpressionCppType) {
    contextualReturn = stdFunctionResultCppType(activeExpectedExpressionCppType);
  }
  ...
  const functionType = activeExpressionTypes.get(expression as Node);
  const analyzedReturn = functionType instanceof FunctionType ? (functionType as FunctionType).returnType : null;
...
function classConstructionTypeSubstitutions(
  ...
  if (classStatement.typeParameters?.length) {
    const typeArguments = resultExpression instanceof NewExpression
      ? (resultExpression as NewExpression).typeArguments
      : resultExpression instanceof CallExpression
...
function emitClassConstruction(
  ...
  if (!className || !activeClassNames.has(className)) {
    throw new CppEmitError(
      `C++ explicit construction does not support '${className ?? String(callee.kind)}' yet${activeSourceFilePath ? ` in ${activeSourceFilePath}` : ""}`
    );
  }
  const classStatement = activeClassStatements.get(className)!;
  ...
  return `vexa::makeManaged<${constructedType}>(${nativeArguments})`;
}
...
function emitTimerCallback(expression: Expr, argumentsList: readonly Expr[]): string {
  ...
  const capture = [`__vexa_timer_callback = ${callback}`, ...captures].join(", ");
  const argumentsText = orderedArguments.map((_, index) => `__vexa_timer_argument_${index}`);
  const invocation = dynamic
    ? `vexa::call(__vexa_timer_callback, {${argumentsText.map((argument) =>
        `vexa::toValue(${argument})`).join(", ")}})`
...
function emitPromiseCall(call: CallExpression): string {
  if (call.args.length !== 1 || !(call.args[0] instanceof ArrowFunctionExpression)) {
  ...
  if (
    ...
    executor.parameters.some((parameter) => !(parameter.name instanceof Identifier))
  ) {
  ...
  const valueType = promiseType instanceof NamedType && promiseType.name === "Promise"
    ? cppTypeForAnalysisType(namedTypeArgument(promiseType, 0) ?? builtinType("unknown")) ?? "vexa::Value"
    : "vexa::Value";
  ...
  try {
    ...
    activeThisExpression = capture.thisExpression;
    const body = executor.body instanceof BlockStatement
      ? emitBlock(executor.body as BlockStatement, "")
      : `{ ${emitExpression(executor.body as Expr)}; }`;
    return `vexa::createTask<${valueType}>(${capture.text}(auto ${cppName(parameterNames[0]!)}, auto ${cppName(parameterNames[1]!)}) mutable ${body})`;
  } finally {
    activeLocalNames = previousLocalNames;
    activeThisExpression = previousThisExpression;
  }
...
function extensionReceiverNamesForExpression(expression: Expr): string[] {
  ...
  if (type instanceof ArrayType || type instanceof TupleType) return ["Array"];
  if (type instanceof BuiltinType) return type.name === "int" ? ["int", "number"] : [type.name];
...
function extensionTemplateArguments(
  ...
  const receiverArguments = receiverType instanceof ArrayType || receiverType instanceof TupleType
    ? [receiverType instanceof ArrayType
        ? receiverType.elementType
        : receiverType.elements[0] ?? builtinType("unknown")]
    : receiverType instanceof NamedType
  ...
  receiverParameters.forEach((parameter, index) => {
    const mapped = receiverArguments[index]
      ? (receiverType instanceof ArrayType || receiverType instanceof TupleType
...
function primitiveRuntimeMethodName(name: string): string | null {
  switch (name) {
    case "toString": case "valueOf": case "toFixed": case "toExponential": case "toPrecision":
    case "toLocaleString": case "toUpperCase":
    ...
    case "substring": case "split": case "concat": case "anchor": case "big": case "blink":
    case "bold": case "fixed": case "fontcolor": case "fontsize": case "italics": case "link":
    case "localeCompare": case "normalize": case "padEnd": case "padStart": case "small":
    case "strike": case "sub": case "substr": case "sup":
      return name;
    ...
    case "match": return "stringMatch";
    case "matchAll": return "stringMatchAll";
    case "search": return "stringSearch";
...
function emitNativeBufferConstruction(name: string, args: readonly Expr[], expression: Node): string | null {
  if (name === "ArrayBuffer") {
    if (args.length < 1 || args.length > 2) throw new CppEmitError("C++ ArrayBuffer construction expects a byte length and optional maxByteLength", expression);
    const maximum = args[1] ? objectLiteralPropertyValue(args[1]!, "maxByteLength") : null;
    return `vexa::makeArrayBuffer(${emitExpression(args[0]!)}${maximum ? `, ${emitExpression(maximum)}` : ""})`;
  }
  ...
  if (nativeTypedArrayNames.has(name)) {
    if (args.length > 3) throw new CppEmitError(`C++ ${name} construction expects a length, iterable, or buffer with optional offset and length`, expression);
    const emitted = args.map((argument) => isManagedArrayExpression(argument)
      ? emitManagedArrayPointer(argument)
      : emitExpression(argument)).join(", ");
    const kind = name.slice(0, -"Array".length);
    return `vexa::makeTypedArray<vexa::TypedArrayKind::${kind}>(${emitted})`;
  }
  if (name === "DataView") {
...
function emitNativeErrorConstruction(name: string, args: readonly Expr[], expression: Node): string {
  if (name === "AggregateError") {
    if (args.length < 1 || args.length > 3) {
      throw new CppEmitError("C++ AggregateError construction expects errors, optional message, and optional options", expression);
    }
    const emitted = args.map((argument) => emitConvertedValue(argument, "vexa::Value"));
    return `vexa::makeAggregateError(${emitted.join(", ")})`;
  }
  if (args.length > 2) {
    throw new CppEmitError(`C++ ${name} construction expects an optional message and options`, expression);
  }
  const emitted = args.map((argument) => emitConvertedValue(argument, "vexa::Value"));
  return `vexa::makeError(${cppUtf16String(name)}${emitted.length > 0 ? `, ${emitted.join(", ")}` : ""})`;
}

function emitNativeIntlConstruction(name: string, args: readonly Expr[], expression: Node): string {
  if (!nativeIntlObjectNames.has(name)) {
    throw new CppEmitError(`Unknown native Intl constructor '${name}'`, expression);
  }
  const minimum = name === "Locale" ? 1 : 0;
  if (args.length < minimum || args.length > 2) {
    throw new CppEmitError(`C++ Intl.${name} expects ${name === "Locale" ? "a tag" : "optional locales"} and optional options`, expression);
  }
  const locales = args[0]
    ? emitConvertedValue(args[0], "vexa::Value")
    : "vexa::Value::undefined()";
  const options = args[1]
    ? emitConvertedValue(args[1], "vexa::Value")
    : "vexa::Value::undefined()";
  return `vexa::makeManaged<vexa::IntlObject>(vexa::IntlObjectKind::${name}, ${locales}, ${options})`;
}
...
function emitCall(call: CallExpression, resultUsed = true): string {
  ...
  if (calleeName && isNativeErrorTypeName(calleeName)) {
    return emitNativeErrorConstruction(calleeName, call.args, call);
  }
  if (calleeName === "Promise") return emitPromiseCall(call);
  if (calleeName === "Object") {
    if (call.args.length > 1) throw new CppEmitError("C++ Object construction expects zero or one value", call);
    return call.args[0]
      ? `vexa::objectConstructor(${emitExpression(call.args[0])})`
      : "vexa::objectConstructor()";
  }
  ...
  const textEncoderObject = member?.object instanceof CallExpression
    ...
    : null;
  const textEncoderCallee = textEncoderObject?.callee;
  if (
    member?.propertyName === "encode" &&
    textEncoderObject !== null &&
    (textEncoderCallee !== undefined && identifierName(textEncoderCallee) === "TextEncoder" ||
      (textEncoderCallee instanceof Identifier && textEncoderCallee.__vexaNativeOriginalName === "TextEncoder"))
  ) {
    if (call.args.length !== 1) throw new CppEmitError("C++ TextEncoder.encode expects one string", call);
    return `vexa::makeUint8Array(vexa::toText(${emitExpression(call.args[0]!)}))`;
  }
  ...
  if (member?.objectName === "Intl" && member.propertyName === "getCanonicalLocales") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Intl.getCanonicalLocales expects locales", call);
    return `vexa::intlCanonicalLocales(${emitConvertedValue(call.args[0]!, "vexa::Value")})`;
  }
  if (member?.objectName === "Intl" && member.propertyName === "supportedValuesOf") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Intl.supportedValuesOf expects a key", call);
    return `vexa::intlSupportedValuesOf(vexa::toText(${emitExpression(call.args[0]!) }))`;
  }
  if (member?.objectName === "Intl" && nativeIntlObjectNames.has(member.propertyName)) {
    return emitNativeIntlConstruction(member.propertyName, call.args, call);
  }
  if (member?.objectName === "Intl" && member.propertyName === "DurationFormat") {
    const args = call.args;
    if (args.length > 2) throw new CppEmitError("C++ Intl.DurationFormat expects optional locales and options", call);
    const locale = args[0] ? `vexa::toText(${emitExpression(args[0])})` : `std::u16string(u"en")`;
    const options = args[1] ? emitExpression(args[1]) : "vexa::Runtime::record()";
    return `vexa::makeManaged<vexa::DurationFormatObject>(${locale}, ${options})`;
  }
  const staticIntlConstructor = member ? intlConstructorName(member.object) : null;
  if (staticIntlConstructor && member?.propertyName === "supportedLocalesOf") {
    if (call.args.length < 1 || call.args.length > 2) {
      throw new CppEmitError(`C++ Intl.${staticIntlConstructor}.supportedLocalesOf expects locales and optional options`, call);
    }
    return `vexa::intlSupportedLocales(${emitConvertedValue(call.args[0]!, "vexa::Value")})`;
  }
  ...
  if (member?.objectName && nativeTypedArrayNames.has(member.objectName) && member.propertyName === "of") {
    const kind = member.objectName.slice(0, -"Array".length);
    return `vexa::typedArrayOf<vexa::TypedArrayKind::${kind}>(${call.args.map((argument) => emitExpression(argument)).join(", ")})`;
  }
  if (member?.objectName && nativeTypedArrayNames.has(member.objectName) && member.propertyName === "from") {
    if (call.args.length < 1 || call.args.length > 2) throw new CppEmitError(`C++ ${member.objectName}.from expects an iterable and optional mapper`, call);
    const source = isManagedArrayExpression(call.args[0]!)
      ? emitManagedArrayPointer(call.args[0]!)
      : emitExpression(call.args[0]!);
    const kind = member.objectName.slice(0, -"Array".length);
    if (!call.args[1]) return `vexa::typedArrayFrom<vexa::TypedArrayKind::${kind}>(${source})`;
    const previousParameters = activeExpectedLambdaParameterCppTypes;
    const previousResult = activeExpectedLambdaResultCppType;
    const elementType = member.objectName.startsWith("Big") ? "vexa::BigInt" : "double";
    activeExpectedLambdaParameterCppTypes = [elementType, "double"];
    activeExpectedLambdaResultCppType = elementType;
    try {
      return `vexa::typedArrayFrom<vexa::TypedArrayKind::${kind}>(${source}, ${emitExpression(call.args[1]!)})`;
    } finally {
      activeExpectedLambdaParameterCppTypes = previousParameters;
      activeExpectedLambdaResultCppType = previousResult;
    }
  ...
  if (member?.objectName === "Number" && member.propertyName === "isSafeInteger") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Number.isSafeInteger expects one argument", call);
    return `vexa::numberIsSafeInteger(${emitConvertedValue(call.args[0]!, "double")})`;
  }
  if (member?.objectName === "Number" &&
      (member.propertyName === "parseFloat" || member.propertyName === "parseInt")) {
    return `vexa::${member.propertyName}(${argumentsText()})`;
  }
  ...
  if (member?.objectName === "String" && member.propertyName === "fromCodePoint") {
    return `vexa::stringFromCodePoint(${argumentsText()})`;
  }
  if (member?.objectName === "String" && member.propertyName === "raw") {
    if (call.args.length < 1) throw new CppEmitError("C++ String.raw expects a template object", call);
    const substitutions = call.args.slice(1)
      .map((argument) => emitConvertedValue(argument, "vexa::Value"))
      .join(", ");
    return `vexa::stringRaw(${emitExpression(call.args[0]!)}, vexa::makeArray<vexa::Value>({${substitutions}}))`;
  }
  ...
  if (member && member.propertyName === "compile" &&
      activeExpressionTypes.get(member.object as Node) instanceof NamedType &&
      (activeExpressionTypes.get(member.object as Node) as NamedType).name === "RegExp") {
    if (call.args.length < 1 || call.args.length > 2) throw new CppEmitError("C++ RegExp.compile expects a pattern and optional flags", call);
    return `${emitExpression(member.object)}.compile(${argumentsText()})`;
  }
  ...
  if (member?.objectName === "Object" && member.propertyName === "assign") {
    if (call.args.length < 1) throw new CppEmitError("C++ Object.assign expects a target", call);
    return `vexa::objectAssign(${call.args.map((argument) => emitExpression(argument)).join(", ")})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "create") {
    if (call.args.length < 1 || call.args.length > 2) throw new CppEmitError("C++ Object.create expects a prototype and optional descriptors", call);
    const prototype = emitConvertedValue(call.args[0]!, "vexa::Value");
    return call.args[1]
      ? `vexa::objectCreate(${prototype}, ${emitExpression(call.args[1])})`
      : `vexa::objectCreate(${prototype})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "defineProperties") {
    if (call.args.length !== 2) throw new CppEmitError("C++ Object.defineProperties expects an object and descriptors", call);
    return `vexa::objectDefineProperties(${emitExpression(call.args[0]!)}, ${emitExpression(call.args[1]!)})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "getOwnPropertyDescriptor") {
    if (call.args.length !== 2) throw new CppEmitError("C++ Object.getOwnPropertyDescriptor expects an object and key", call);
    return `vexa::objectGetOwnPropertyDescriptor(${emitExpression(call.args[0]!)}, vexa::propertyKey(${emitExpression(call.args[1]!)}))`;
  }
  if (member?.objectName === "Object" && member.propertyName === "getOwnPropertyDescriptors") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Object.getOwnPropertyDescriptors expects an object", call);
    return `vexa::objectGetOwnPropertyDescriptors(${emitExpression(call.args[0]!)})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "getOwnPropertyNames") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Object.getOwnPropertyNames expects an object", call);
    return `vexa::objectGetOwnPropertyNames(${emitExpression(call.args[0]!)})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "getPrototypeOf") {
    if (call.args.length !== 1) throw new CppEmitError("C++ Object.getPrototypeOf expects an object", call);
    return `vexa::objectGetPrototypeOf(${emitExpression(call.args[0]!)})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "setPrototypeOf") {
    if (call.args.length !== 2) throw new CppEmitError("C++ Object.setPrototypeOf expects an object and prototype", call);
    const target = emitExpression(call.args[0]!);
    const prototype = emitConvertedValue(call.args[1]!, "vexa::Value");
    return `([&]() { auto __vexa_target = ${target}; vexa::objectSetPrototypeOf(vexa::convertValue<vexa::Value>(__vexa_target), ${prototype}); return __vexa_target; }())`;
  }
  if (member?.objectName === "Object" && member.propertyName === "hasOwn") {
    if (call.args.length !== 2) throw new CppEmitError("C++ Object.hasOwn expects an object and key", call);
    return `vexa::objectHasOwn(${emitExpression(call.args[0]!)}, vexa::propertyKey(${emitExpression(call.args[1]!)}))`;
  }
  if (member?.objectName === "Object" && member.propertyName === "is") {
    if (call.args.length !== 2) throw new CppEmitError("C++ Object.is expects two values", call);
    return `vexa::objectIs(${emitExpression(call.args[0]!)}, ${emitExpression(call.args[1]!)})`;
  }
  if (member?.objectName === "Object" &&
      new Set(["freeze", "seal", "preventExtensions", "isFrozen", "isSealed", "isExtensible"]).has(member.propertyName)) {
    if (call.args.length !== 1) throw new CppEmitError(`C++ Object.${member.propertyName} expects an object`, call);
    const helper = `object${member.propertyName[0]!.toUpperCase()}${member.propertyName.slice(1)}`;
    return `vexa::${helper}(${emitExpression(call.args[0]!)})`;
  }
  if (member?.objectName === "Object" &&
  ...
  if (member?.objectName === "Object" && member.propertyName === "fromEntries") {
    ...
    return `vexa::recordFromEntries(${emitExpression(call.args[0]!)})`;
  }
  ...
  if (member?.objectName === "Date" && member.propertyName === "UTC") {
    if (call.args.length < 2 || call.args.length > 7) throw new CppEmitError("C++ Date.UTC expects between two and seven components", call);
    return `vexa::dateUTC(${argumentsText()})`;
  }
  if (member?.objectName === "Object" &&
    ...
    return `vexa::record${member.propertyName === "keys" ? "Keys" : "Values"}(${emitExpression(call.args[0]!)})`;
  }
  if (member?.objectName === "Object" && member.propertyName === "defineProperty") {
    ...
    return `vexa::objectDefineProperty(${emitExpression(target)}, vexa::propertyKey(${emitExpression(key)}), ${emitConvertedValue(call.args[2]!, "vexa::Value")})`;
  }
  ...
  if (member?.objectName === "Promise") {
    ...
    if (member.propertyName === "resolve") {
      ...
      return call.args.length === 0
        ? `vexa::promiseResolve(vexa::Value::undefined())`
        : `vexa::promiseResolve(${emitExpression(call.args[0]!)})`;
    }
    if (member.propertyName === "reject") {
      ...
      const contextualValueType = activeExpectedExpressionCppType
        ...
        : null;
      const valueType = contextualValueType ?? (promiseType instanceof NamedType && promiseType.name === "Promise"
        ? cppTypeForAnalysisType(namedTypeArgument(promiseType, 0) ?? builtinType("unknown")) ?? "vexa::Value"
        : "vexa::Value");
      return `vexa::rejectedTask<${valueType}>(${emitExpression(call.args[0]!)})`;
    }
    ...
    if (promiseCombinator) {
      ...
      return `vexa::${promiseCombinator}(${tasks})`;
    }
  }
  if (member && new Set(["then", "catch", "finally"]).has(member.propertyName)) {
    if (call.args.length !== 1) {
    ...
    let result: string;
    try {
      result = `vexa::${helper}(${emitExpression(member.object)}, ${emitExpression(call.args[0]!)})`;
    } finally {
      activeExpectedLambdaParameterCppTypes = previousParameters;
      activeExpectedLambdaResultCppType = previousResult;
    }
  ...
  if (member) {
    const receiverType = emittedCppTypeForExpression(member.object) ?? cppTypeForExpression(member.object);
    const receiverAnalysis = activeExpressionTypes.get(member.object as Node);
    const isObjectPrototypeReceiver = receiverType === "vexa::RecordObject*" ||
      receiverAnalysis instanceof ObjectType ||
      (receiverAnalysis instanceof NamedType && receiverAnalysis.name === "Object");
    if (isObjectPrototypeReceiver) {
      const receiver = emitExpression(member.object);
      if (member.propertyName === "hasOwnProperty" || member.propertyName === "propertyIsEnumerable") {
        if (call.args.length !== 1) throw new CppEmitError(`C++ Object.${member.propertyName} expects one property key`, call);
        const helper = member.propertyName === "hasOwnProperty" ? "objectHasOwn" : "objectPropertyIsEnumerable";
        return `vexa::${helper}(${receiver}, vexa::propertyKey(${emitExpression(call.args[0]!)}))`;
      }
      if (member.propertyName === "isPrototypeOf") {
        if (call.args.length !== 1) throw new CppEmitError("C++ Object.isPrototypeOf expects one object", call);
        return `vexa::objectIsPrototypeOf(${receiver}, ${emitExpression(call.args[0]!)})`;
      }
      if (member.propertyName === "toString" || member.propertyName === "toLocaleString") {
        if (call.args.length !== 0) throw new CppEmitError(`C++ Object.${member.propertyName} expects no arguments`, call);
        return `vexa::objectToString(${receiver})`;
      }
      if (member.propertyName === "valueOf") {
        if (call.args.length !== 0) throw new CppEmitError("C++ Object.valueOf expects no arguments", call);
        return receiver;
      }
    }
  }
  if (member) {
    ...
    if (collection === "map") {
        ...
        return emitNativeReceiverCall(
          ...
          (target) => {
            const helper = member.propertyName === "get"
              ? getResultType?.endsWith("*") && activeNativeCollectionRepresentation === "specialized" ? "mapGet" : "mapGetValue"
              : member.propertyName === "has" ? "mapHas" : "mapDelete";
            const emitted = `vexa::${helper}(${target}, ${emitExpression(call.args[0]!)})`;
            return member.propertyName === "get" && activeNativeCollectionRepresentation === "generic" &&
                getResultType?.endsWith("*")
      ...
      if (member.propertyName === "forEach") {
        ...
        if (activeNativeCollectionRepresentation === "specialized") {
        ...
        const receiverType = nativeCollectionPointerCppType(member.object) ?? "vexa::MapObject<vexa::Value, vexa::Value>*";
        const callbackTypes = [semanticTypes[1] ?? "vexa::Value", semanticTypes[0] ?? "vexa::Value", receiverType];
        ...
        const adapter = `[__vexa_callback = ${emittedCallback}](vexa::Value __vexa_value, vexa::Value __vexa_key, ${receiverType} __vexa_map) mutable { __vexa_callback(${forwarded.join(", ")}); }`;
        return emitNativeReceiverCall(optionalReceiver, receiver, (target) =>
      ...
      if (member.propertyName === "keys" || member.propertyName === "values" || member.propertyName === "entries") {
        ...
        const actualType = member.propertyName === "entries"
          ? "vexa::ArrayObject<vexa::ArrayObject<vexa::Value>*>*"
          : activeNativeCollectionRepresentation === "generic"
    ...
    if (collection === "set") {
      ...
      if (member.propertyName === "forEach") {
        ...
        const receiverType = nativeCollectionPointerCppType(member.object) ?? "vexa::SetObject<vexa::Value>*";
        const callbackTypes = [elementType, elementType, receiverType];
        const parameterCount = callableParameterCount(callback);
        const previousParameters = activeExpectedLambdaParameterCppTypes;
        activeExpectedLambdaParameterCppTypes = callbackTypes.slice(0, parameterCount);
        let emittedCallback: string;
        ...
        const adapter = `[__vexa_callback = ${emittedCallback}](vexa::Value __vexa_value, vexa::Value, ${receiverType} __vexa_set) mutable { __vexa_callback(${forwarded.join(", ")}); }`;
        return emitNativeReceiverCall(optionalReceiver, receiver, (target) =>
      ...
      if (member.propertyName === "keys" || member.propertyName === "values") {
        ...
        return emitNativeReceiverCall(optionalReceiver, receiver, (target) => {
          ...
          return activeNativeCollectionRepresentation === "specialized" || elementType === "vexa::Value"
            ? emitted
    ...
    if (collection === "weakMap") {
      ...
      if (member.propertyName === "get" || member.propertyName === "has" || member.propertyName === "delete") {
        ...
        return emitNativeReceiverCall(optionalReceiver, receiver, (target) => {
          ...
          return activeNativeCollectionRepresentation === "generic" && resultType && resultType !== "vexa::Value" && resultType !== "auto"
            ? emitNativeConversion(emitted, resultType)
            : emitted;
        });
      }
  ...
  if (member && isDateExpression(member.object)) {
    const supported = new Set([
      ...
      "getMinutes", "getSeconds", "getMilliseconds", "getTimezoneOffset", "getUTCFullYear",
      "getUTCMonth", "getUTCDate", "getUTCDay", "getUTCHours", "getUTCMinutes", "getUTCSeconds",
      "getUTCMilliseconds", "setTime", "setMilliseconds", "setSeconds", "setMinutes", "setHours",
      "setDate", "setMonth", "setFullYear", "setUTCMilliseconds", "setUTCSeconds", "setUTCMinutes",
      "setUTCHours", "setUTCDate", "setUTCMonth", "setUTCFullYear", "toISOString", "toJSON",
  ...
  if (member &&
      (emittedCppTypeForExpression(member.object) === "vexa::IntlObject*" ||
       cppTypeForExpression(member.object) === "vexa::IntlObject*")) {
    const supported = new Set([
      "compare", "format", "formatToParts", "formatRange", "formatRangeToParts",
      "maximize", "minimize", "of", "resolvedOptions", "segment", "select",
      "selectRange", "toString",
    ]);
    if (supported.has(member.propertyName)) {
      const args = call.args.map((argument) => isManagedArrayExpression(argument)
        ? emitManagedArrayPointer(argument)
        : emitExpression(argument));
      if ((member.propertyName === "format" || member.propertyName === "formatToParts") && args.length === 0) {
        args.push("0.0");
      }
      return `${emitExpression(member.object)}->${cppName(member.propertyName)}(${args.join(", ")})`;
    }
  }
  if (member &&
      (emittedCppTypeForExpression(member.object) === "vexa::IntlSegmentsObject*" ||
       cppTypeForExpression(member.object) === "vexa::IntlSegmentsObject*") &&
      member.propertyName === "containing") {
    if (call.args.length > 1) throw new CppEmitError("C++ Intl.Segments.containing expects zero or one index", call);
    return `${emitExpression(member.object)}->containing(${call.args[0] ? emitExpression(call.args[0]) : "0.0"})`;
  }
  if (member && nativeBinaryObjectKind(member.object) === "dataView") {
    const supported = new Set([
      ...
      "setFloat16", "setFloat32", "setFloat64", "setBigInt64", "setBigUint64",
    ]);
    if (supported.has(member.propertyName)) {
      return `${emitExpression(member.object)}->${cppName(member.propertyName)}(${argumentsText()})`;
    }
  ...
  if (member && nativeBinaryObjectKind(member.object) === "buffer" && member.propertyName === "resize") {
    if (call.args.length > 1) throw new CppEmitError("C++ ArrayBuffer.resize expects zero or one length", call);
    return `${emitExpression(member.object)}->resize(${argumentsText()})`;
  }
  if (member && nativeBinaryObjectKind(member.object) === "buffer" &&
      (member.propertyName === "transfer" || member.propertyName === "transferToFixedLength")) {
    if (call.args.length > 1) throw new CppEmitError(`C++ ArrayBuffer.${member.propertyName} expects zero or one length`, call);
    return `${emitExpression(member.object)}->${member.propertyName}(${argumentsText()})`;
  }
  ...
  if (member && nativeBinaryObjectKind(member.object) === "typedArray") {
    const receiverType = emittedCppTypeForExpression(member.object) ?? cppTypeForExpression(member.object);
    const elementType = receiverType.includes("BigInt") || receiverType.includes("BigUint")
      ? "vexa::BigInt"
      : "double";
    const callbackMethods = new Set([
      "every", "filter", "find", "findIndex", "findLast", "findLastIndex", "forEach",
      "map", "reduce", "reduceRight", "some", "sort", "toSorted",
    ]);
    const previousParameters = activeExpectedLambdaParameterCppTypes;
    const previousResult = activeExpectedLambdaResultCppType;
    if (callbackMethods.has(member.propertyName) && call.args.length > 0) {
      if (member.propertyName === "reduce" || member.propertyName === "reduceRight") {
        const semanticResultType = cppTypeForExpression(call);
        const resultType = semanticResultType !== "auto"
          ? semanticResultType
          : call.args[1]
            ? emittedCppTypeForExpression(call.args[1]!) ?? cppTypeForExpression(call.args[1]!)
            : elementType;
        activeExpectedLambdaParameterCppTypes = [resultType, elementType, "double", receiverType];
        activeExpectedLambdaResultCppType = resultType;
      } else if (member.propertyName === "sort" || member.propertyName === "toSorted") {
        activeExpectedLambdaParameterCppTypes = [elementType, elementType];
        activeExpectedLambdaResultCppType = "double";
      } else {
        activeExpectedLambdaParameterCppTypes = [elementType, "double", receiverType];
        activeExpectedLambdaResultCppType = member.propertyName === "map"
          ? elementType
          : member.propertyName === "forEach"
            ? "void"
            : "bool";
      }
    }
    let typedArrayArguments: string;
    try {
      typedArrayArguments = call.args.map((argument, index) => {
        if ((member.propertyName === "reduce" || member.propertyName === "reduceRight") && index === 1) {
          return emitConvertedValue(argument, activeExpectedLambdaResultCppType ?? elementType);
        }
        if ((member.propertyName === "fill" && index === 0) ||
            (member.propertyName === "with" && index === 1)) {
          return emitConvertedValue(argument, elementType);
        }
        return isManagedArrayExpression(argument) ? emitManagedArrayPointer(argument) : emitExpression(argument);
      }).join(", ");
    } finally {
      activeExpectedLambdaParameterCppTypes = previousParameters;
      activeExpectedLambdaResultCppType = previousResult;
    }
    return `${emitExpression(member.object)}->${cppName(member.propertyName)}(${typedArrayArguments})`;
  }
  if (member && isArrayExpression(member.object) && isArrayRuntimeMethod(member.propertyName)) {
    ...
    let receiverElementType = managedArrayElementType(receiverType);
    if (!receiverElementType && isDynamicValueExpression(member.object)) {
      receiverElementType = "vexa::Value";
    }
    if (member.propertyName === "push" && call.args.some((argument) => argument instanceof SpreadExpression)) {
    ...
    if (member.propertyName === "splice" && call.args.slice(2).some((argument) => argument instanceof SpreadExpression)) {
      ...
      const body = `auto* __vexa_receiver = ${receiver}; auto* __vexa_values = ${packed};`;
      const splice = `vexa::spliceAll(__vexa_receiver, ${emitExpression(call.args[0]!)}, ${emitExpression(call.args[1]!)}, __vexa_values)`;
    ...
    const emitContextualArrayArguments = (): string => {
      ...
      if (member.propertyName === "map" && contextualCallbackResult) {
        ...
        activeExpectedLambdaResultCppType = mappedCallType !== "auto"
          ? mappedCallType
          : initial
      ...
      let result: string;
      try {
        result = call.args.map((argument, index) =>
          index === 1 && (member.propertyName === "reduce" || member.propertyName === "reduceRight") &&
            activeExpectedLambdaResultCppType
            ? emitConvertedValue(argument, activeExpectedLambdaResultCppType)
            : emitExpression(argument)
        ).join(", ");
      } finally {
        activeExpectedExpressionCppType = previous;
    ...
    return emitNativeReceiverCall(optionalReceiver, receiver, (target) =>
      `vexa::${member.propertyName}(${target}${arrayArguments ? `, ${arrayArguments}` : ""})`
    );
  }
  ...
  if (member) {
    ...
    if (primitiveMethod) {
      ...
      if (primitiveMethod === "toString" && call.args.length > 1) {
        throw new CppEmitError("C++ toString expects at most one radix", call);
      } else if (numericArguments) {
        emittedArguments = call.args.map((argument) => emitConvertedValue(argument, "double")).join(", ");
      } else if (primitiveMethod === "toString" && call.args.length === 1) {
        emittedArguments = emitConvertedValue(call.args[0]!, "double");
      } else {
  ...
  if (member && isDynamicValueExpression(member.object) &&
    ...
    return `vexa::${optional ? "callOptional" : "call"}(${callable}, {${dynamicArguments.join(", ")}})`;
  }
  ...
  if (member) {
    ...
    if (method) {
      ...
      const classMethod = method instanceof ClassMethodMember ? method as ClassMethodMember : null;
  ...
  if (calleeName === "nativeReadFileBytes" || calleeName === "nativeRealPath" || calleeName === "nativeStatPath" || calleeName === "nativeReadDirectory") {
    ...
    return `vexa::${calleeName}(vexa::toText(${emitExpression(call.args[0]!)}))`;
  }
  if (calleeName === "nativeCreateDirectory" || calleeName === "nativeRemovePath") {
    ...
    return `vexa::${calleeName}(vexa::toText(${emitExpression(call.args[0]!)}), vexa::toBoolean(${emitExpression(call.args[1]!)}))`;
  }
  if (calleeName === "nativeCopyFile") {
    if (call.args.length !== 2) {
      throw new CppEmitError("C++ nativeCopyFile expects source and target paths");
    }
    return `vexa::nativeCopyFile(vexa::toText(${emitExpression(call.args[0]!)}), vexa::toText(${emitExpression(call.args[1]!)}))`;
  }
  if (calleeName === "nativeRunCommandCapture") {
    ...
    return `vexa::nativeRunCommandCapture(vexa::toString(${emitExpression(call.args[0]!)}), vexa::toInstance<vexa::ArrayObject<std::u16string>*>(${emitExpression(call.args[1]!)}), vexa::toString(${emitExpression(call.args[2]!)}))`;
  }
  ...
  const runtimeGlobals = new Set([
    "String", "Number", "Boolean", "BigInt", "parseInt", "parseFloat", "isNaN", "isFinite",
    "encodeURI", "decodeURI", "encodeURIComponent", "decodeURIComponent", "escape", "unescape",
  ]);
  if (calleeName && isNativeErrorTypeName(calleeName)) {
    return `vexa::Error(${argumentsText()})`;
  }
  ...
  const currentClassMethod = calleeName && currentClass && !activeLocalNames.has(calleeName)
    ? currentClass.members.find((candidate): candidate is ClassMethodMember =>
        candidate instanceof ClassMethodMember && candidate.name.name === calleeName) ??
  ...
  const dynamicRecordCallable = call.callee instanceof MemberExpression &&
    activeExpressionTypes.get((call.callee as MemberExpression).object as Node) instanceof ObjectType;
  const resolvedCallableProperty: NativePropertyMember | null = call.callee instanceof MemberExpression
  ...
  if (isDynamicValueExpression(call.callee) || dynamicRecordCallable || dynamicPropertyCallable) {
    ...
    const dynamicCallee = resolvedCallableProperty?.receiver
      ? `vexa::dynamicGet(vexa::toValue(${emitExpression(resolvedCallableProperty.receiver)}), ${emitNativePropertyKey(resolvedCallableProperty)})`
      : emitExpression(call.callee);
    const invocation = `vexa::${call.optional ? "callOptional" : "call"}(${dynamicCallee}, {${dynamicArguments.join(", ")}})`;
    const resultType = emittedCppTypeForExpression(call);
    return resultType && resultType !== "auto" && resultType !== "void" && resultType !== "vexa::Value"
      ? emitNativeConversion(invocation, resultType)
      : invocation;
  }
  return `${emitExpression(call.callee)}(${argumentsText()})`;
}
...
function emitPointerNullishEquality(expression: BinaryExpression): string | null {
  ...
  const leftNullish = expression.left instanceof NullLiteral || expression.left instanceof UndefinedLiteral;
  const rightNullish = expression.right instanceof NullLiteral || expression.right instanceof UndefinedLiteral;
...
function emitDynamicBinaryText(operator: string, left: string, right: string): string | null {
  ...
  const convertedLeft = `vexa::toValue(${left})`;
  const convertedRight = `vexa::toValue(${right})`;
  return helper === "add"
    ? `vexa::add(${left}, ${right})`
...
function emitBinary(expression: BinaryExpression): string {
  ...
  if (expression.operator === "??") {
    ...
    if (leftType === "vexa::Value" || resultType === "vexa::Value") {
      ...
      const fallback = resultType && resultType !== "vexa::Value"
        ? `vexa::toValue(${emitExpressionWithExpectedCppType(expression.right, resultType)})`
        : emitConvertedValue(expression.right, "vexa::Value");
      const coalesced = `vexa::nullishCoalesce(${convertedLeft}, [&]() { return ${fallback}; })`;
      return resultType && resultType !== "vexa::Value"
        ? emitNativeConversion(coalesced, resultType)
        : coalesced;
    }
  ...
  if (expression.operator === "&&" || expression.operator === "||") {
    ...
    return logicalResultType !== "auto" && logicalResultType !== "vexa::Value"
      ? emitNativeConversion(selected, logicalResultType)
      : selected;
  }
  if (expression.operator === "<=>") {
  ...
  if (expression.operator === "in") {
    return `vexa::hasProperty(${emitExpression(expression.right)}, vexa::propertyKey(${emitExpression(expression.left)}))`;
  }
  if ((expression.operator === "is" || expression.operator === "instanceof") && expression.right instanceof Identifier) {
    ...
    if (targetName === "ArrayBuffer" || targetName === "Uint8Array" || targetName === "DataView") {
      return `vexa::isInstance<vexa::${targetName}Object>(${emitExpression(expression.left)})`;
    }
    const targetType = cppTypeForDeclaredName(targetName);
    if (targetType?.endsWith("*") &&
      (activeClassNames.has(parseTypeNameShape(targetName).baseName) ||
        activeInterfaceNames.has(parseTypeNameShape(targetName).baseName))) {
      const sourceName = identifierName(expression.left);
      const temporary = sourceName
        ? activeInstanceofTemporaryExpressions.get(instanceofNarrowingKey(sourceName, targetName))
        : undefined;
      if (temporary) return `(${temporary} != nullptr)`;
  ...
  if (operator === "is" || operator === "instanceof") {
    throw new CppEmitError(
      `C++ emission does not support '${operator} ${identifierName(expression.right) ?? nodeKindName(expression.right.kind)}' yet${activeSourceFilePath ? ` in ${activeSourceFilePath}` : ""}`
    );
  }
  return `(${emitExpression(expression.left)} ${operator} ${emitExpression(expression.right)})`;
}
...
function emitCondition(expression: Expr): string {
  ...
  if (expression instanceof Identifier && activeDynamicValueNames.has((expression as Identifier).name)) {
    return `vexa::Boolean(${emitted})`;
  }
...
function instanceofNarrowingKey(sourceName: string, targetName: string): string {
...
function emitExpressionResult(expression: Expr, resultUsed: boolean): string {
  switch (expression.kind) {
    ...
    case NodeKind.FloatLiteral:
      return cppFloat((expression as FloatLiteral).value);
    case NodeKind.CharacterLiteral:
    ...
    case NodeKind.StringLiteral:
      return pooledStringText((expression as StringLiteral).value);
    case NodeKind.RegExpLiteral: {
    ...
    case NodeKind.UnaryExpression: {
      ...
      if (unary.operator === "typeof") {
        if (usesPooledFunctionTypeof(unary)) return pooledStringText("function");
    ...
    case NodeKind.UpdateExpression: {
      ...
      const variableDelegate = update.argument instanceof Identifier
        ? activeVariableDelegates.get(update.argument.name)
        : undefined;
      if (variableDelegate) {
        const currentName = `__vexa_delegate_current_${activeDelegateTemporaryCounter++}`;
        const valueName = `__vexa_delegate_value_${activeDelegateTemporaryCounter++}`;
        const delta = update.operator === "++" ? "+" : "-";
        const write = emitVariableDelegateWrite(variableDelegate, valueName);
        return `([&]() { auto ${currentName} = ${emitExpression(update.argument)}; auto ${valueName} = (${currentName} ${delta} 1); ${write}; return ${update.prefix ? valueName : currentName}; }())`;
      }
      if (update.argument instanceof MemberExpression) {
    ...
    case NodeKind.AssignmentExpression: {
      ...
      const variableDelegate = assignment.left instanceof Identifier
        ? activeVariableDelegates.get(assignment.left.name)
        : undefined;
      if (variableDelegate) {
        if (assignment.operator === "=") {
          return emitVariableDelegateWrite(variableDelegate, emitExpression(assignment.right));
        }
        const operator = compoundAssignmentBinaryOperator(assignment.operator);
        if (!operator) throw new CppEmitError(`C++ variable delegates do not support '${assignment.operator}'`, assignment);
        return emitVariableDelegateWrite(
          variableDelegate,
          `(${emitExpression(assignment.left)} ${operator} ${emitExpression(assignment.right)})`
        );
      }
      if (overloaded?.operator === "[]=" && assignment.left instanceof MemberExpression) {
      ...
      if (assignment.left instanceof MemberExpression) {
        ...
        if (member.computed) {
          const binaryKind = nativeBinaryObjectKind(member.object);
          if (binaryKind === "typedArray") {
            if (assignment.operator !== "=") throw new CppEmitError("C++ typed-array compound index assignment is not implemented yet", assignment);
      ...
      if (assignment.operator === "+=" && isDynamicValueExpression(assignment.left)) {
        return `vexa::addAssign(${emitExpression(assignment.left)}, ${emitExpression(assignment.right)})`;
      }
      ...
      if (assignment.operator === "%=") {
        const target = emitExpression(assignment.left);
        const value = emitExpression(assignment.right);
        return `vexa::assignWith(${target}, [&](auto __vexa_compound_current) { return vexa::remainder(__vexa_compound_current, ${value}); })`;
      }
      if (compoundOperator) {
        const targetType = emittedCppTypeForExpression(assignment.left) ?? cppTypeForExpression(assignment.left);
        const resolvedTargetType = assignment.left instanceof Identifier
          ? activeLocalCppTypes.get(assignment.left.name) ?? targetType
          : targetType;
        if (targetType === "std::u16string") {
          ...
          if (value) {
            return `vexa::assignWith(${target}, [&](const std::u16string& __vexa_compound_current) { return vexa::toText(${value}); })`;
          }
        }
        if ((resolvedTargetType === "double" || resolvedTargetType === "std::int32_t" || resolvedTargetType === "std::int64_t") &&
          (cppTypeForExpression(assignment.right) === "vexa::Value" || isDynamicValueExpression(assignment.right))) {
          const target = emitExpression(assignment.left);
          const value = emitConvertedValue(assignment.right, resolvedTargetType);
          return `(${target} ${assignment.operator} ${value})`;
        }
      }
      if (compoundOperator && assignment.left instanceof Identifier) {
        const analyzedTargetType = activeExpressionTypes.get(assignment.left as Node);
        const targetType = activeLocalCppTypes.get(assignment.left.name) ??
          cppTypeForAnalysisType(analyzedTargetType ?? builtinType("unknown")) ??
          cppTypeForExpression(assignment.left);
        if (targetType === "double" || targetType === "std::int32_t" || targetType === "std::int64_t") {
          const target = emitExpression(assignment.left);
          const value = emitConvertedValue(assignment.right, targetType);
          return `(${target} ${assignment.operator} ${value})`;
        }
      }
      if (compoundOperator && !activeHasExpressionTypes) {
        const target = emitExpression(assignment.left);
        const targetType = assignment.left instanceof Identifier
        ...
        if (value) {
          const converted = targetType !== "auto" && targetType !== "vexa::Value"
            ? emitNativeConversion(value, targetType)
            : value;
    ...
    case NodeKind.NewExpression: {
      ...
      if (constructorMember?.objectName === "Intl" && nativeIntlObjectNames.has(constructorMember.propertyName)) {
        return emitNativeIntlConstruction(constructorMember.propertyName, construction.args ?? [], construction);
      }
      ...
      if (collectionName === "Object") {
        const args = construction.args ?? [];
        if (args.length > 1) throw new CppEmitError("C++ Object construction expects zero or one value", construction);
        return args[0]
          ? `vexa::objectConstructor(${emitExpression(args[0])})`
          : "vexa::objectConstructor()";
      }
      if (collectionName === "Date") {
        const dateArguments = construction.args ?? [];
        if (dateArguments.length > 7) throw new CppEmitError("C++ Date construction expects at most seven components", construction);
        const emittedArguments = dateArguments.length === 1 && cppTypeForExpression(dateArguments[0]!) === "vexa::Value"
          ? `vexa::toText(${emitExpression(dateArguments[0]!)})`
          : dateArguments.map((argument) => emitExpression(argument)).join(", ");
        return `vexa::makeManaged<vexa::DateObject>(${emittedArguments})`;
      }
      if (collectionName === "URL") {
        ...
        return `vexa::makeManaged<vexa::URLObject>(vexa::toText(${emitExpression(construction.args![0] as Expr)}))`;
      }
      if (collectionName && isNativeErrorTypeName(collectionName)) {
        return emitNativeErrorConstruction(collectionName, construction.args ?? [], construction);
      }
      ...
      if (collectionName === "Array") {
        ...
        if (explicitElementType.length > 0) elementType = explicitElementType;
          if (args.length === 0) return `vexa::makeArray<${elementType}>()`;
        if (args.length === 1) {
          return `vexa::arrayWithLength<${elementType}>(${emitExpression(args[0] as Expr)})`;
        }
          return `vexa::makeArray<${elementType}>({${emitArrayElements(args, elementType)}})`;
      }
    ...
    case NodeKind.MemberExpression: {
      ...
      if (!member.computed && identifierName(member.object) === "super" && member.property instanceof Identifier) {
        const currentClass = activeCurrentClassStatement ?? (activeCurrentClassName
          ? activeClassStatements.get(activeCurrentClassName)
          : undefined);
      ...
      const enumName = !member.computed ? identifierName(member.object) : null;
      if (enumName && activeEnumNames.has(enumName) && member.property instanceof Identifier) {
        return `${cppName(enumName)}::${cppName((member.property as Identifier).name)}`;
      }
      if (member.computed && member.object instanceof Identifier && member.property instanceof StringLiteral) {
        const enumStatement = activeEnumStatements.get(member.object.name);
        const enumMember = enumStatement
          ? enumStatement.members.find((candidate) =>
            candidate.name.name === (member.property as StringLiteral).value)
          : undefined;
        if (enumMember) return `${cppName(member.object.name)}::${cppName(enumMember.name.name)}`;
      }
      ...
      if (!member.computed && identifierName(member.object) === "Math" && member.property instanceof Identifier) {
        return `vexa::Math::${cppName((member.property as Identifier).name)}`;
      }
      if (identifierName(member.object) === "RegExp" &&
          (member.property instanceof Identifier || member.property instanceof StringLiteral)) {
        const propertyName = member.property instanceof Identifier
          ? (member.property as Identifier).name
          : (member.property as StringLiteral).value;
        const capture = /^\$([1-9])$/.exec(propertyName);
        if (capture) return `vexa::regexLegacyCapture(${Number(capture[1]) - 1})`;
        const helper = new Map<string, string>([
          ["input", "regexLegacyInput"], ["$_", "regexLegacyInput"],
          ["lastMatch", "regexLegacyLastMatch"], ["$&", "regexLegacyLastMatch"],
          ["lastParen", "regexLegacyLastParen"], ["$+", "regexLegacyLastParen"],
          ["leftContext", "regexLegacyLeftContext"], ["$`", "regexLegacyLeftContext"],
          ["rightContext", "regexLegacyRightContext"], ["$'", "regexLegacyRightContext"],
        ]).get(propertyName);
        if (helper) return `vexa::${helper}()`;
      }
      if (!member.computed && cppTypeForExpression(member.object) === "vexa::URLObject*" && member.property instanceof Identifier) {
        const propertyName = (member.property as Identifier).name;
        if (new Set(["href", "protocol", "pathname"]).has(propertyName)) {
          return `vexa::makeString(${emitExpression(member.object)}->${cppName(propertyName)})`;
        }
      }
      if (!member.computed && member.property instanceof Identifier &&
          (emittedCppTypeForExpression(member.object) === "vexa::IntlObject*" ||
           cppTypeForExpression(member.object) === "vexa::IntlObject*")) {
        const propertyName = (member.property as Identifier).name;
        if (new Set(["baseName", "language"]).has(propertyName)) {
          return `vexa::makeString(${emitExpression(member.object)}->${cppName(propertyName)}())`;
        }
        if (new Set(["script", "region", "calendar", "caseFirst", "collation", "hourCycle", "numberingSystem"]).has(propertyName)) {
          return `${emitExpression(member.object)}->${cppName(propertyName)}()`;
        }
        if (propertyName === "numeric") {
          return `${emitExpression(member.object)}->numeric()`;
        }
      }
      ...
      if (!member.computed && !messageReceiverIsStoredDynamic &&
        ...
        return `vexa::makeString(vexa::errorMessageText(${emitExpression(member.object)}))`;
      }
      if (!member.computed && !messageReceiverIsStoredDynamic &&
          messageReceiverStorageType !== "vexa::Value" &&
          emittedCppTypeForExpression(member.object) === "vexa::Error") {
        const propertyName = identifierName(member.property);
        if (propertyName === "name") {
          return `vexa::makeString(vexa::errorNameText(${emitExpression(member.object)}))`;
        }
        if (propertyName === "stack") {
          return `vexa::makeString(vexa::errorStackText(${emitExpression(member.object)}))`;
        }
        if (propertyName === "cause") {
          return `vexa::errorCause(${emitExpression(member.object)})`;
        }
        if (propertyName === "errors") {
          return `vexa::aggregateErrorErrors(${emitExpression(member.object)})`;
        }
      }
      if (!member.computed && identifierName(member.property) === "message" &&
        ...
        return `vexa::dynamicGet(vexa::toValue(${emitExpression(member.object)}), u"message")`;
      }
      if (!member.computed && identifierName(member.property) === "message") {
        const className = classNameForExpression(member.object);
        const statement = className ? activeClassStatements.get(className) : undefined;
        ...
        if (baseName && isNativeErrorTypeName(baseName)) {
          return `vexa::makeString(${emitExpression(member.object)}->messageText())`;
        }
      }
      if (!member.computed && isManagedArrayExpression(member.object) && identifierName(member.property) === "length") {
        ...
        if (member.optional && isManagedArrayExpression(member.object)) {
          const receiver = emitManagedArrayPointer(member.object);
          return `([&]() { auto* __vexa_optional_array = ${receiver}; return __vexa_optional_array ? vexa::toValue(static_cast<double>(__vexa_optional_array->size())) : vexa::Value::undefined(); }())`;
        }
        const receiver = isManagedArrayExpression(member.object)
      ...
      if (!member.computed && member.optional && identifierName(member.property) === "length") {
        return `vexa::dynamicGetOptional(vexa::toValue(${emitExpression(member.object)}), u"length")`;
      }
      ...
      const staticTypedArrayName = identifierName(member.object);
      if (!member.computed && staticTypedArrayName && nativeTypedArrayNames.has(staticTypedArrayName) &&
          identifierName(member.property) === "BYTES_PER_ELEMENT") {
        return `static_cast<double>(${nativeTypedArrayCppType(staticTypedArrayName)!.slice(0, -1)}::bytesPerElement)`;
      }
      if (!member.computed && identifierName(member.object) === "Number" && member.property instanceof Identifier) {
        ...
        ]).get((member.property as Identifier).name);
      ...
      if (!member.computed && binaryKind && member.property instanceof Identifier) {
        ...
        if (binaryKind === "typedArray" && propertyName === "length") return `static_cast<double>(${emitExpression(member.object)}->length())`;
        if (binaryKind === "typedArray" && propertyName === "BYTES_PER_ELEMENT") return `static_cast<double>(${emitExpression(member.object)}->bytesPerElement)`;
        if ((binaryKind === "typedArray" || binaryKind === "dataView") && propertyName === "byteOffset") {
          return `static_cast<double>(${emitExpression(member.object)}->byteOffset())`;
        }
        if ((binaryKind === "typedArray" || binaryKind === "dataView") && propertyName === "buffer") {
          return `${emitExpression(member.object)}->buffer()`;
        }
      }
      if (member.computed && binaryKind === "typedArray") {
        return `${emitExpression(member.object)}->get(static_cast<std::size_t>(${emitExpression(member.property)}))`;
      }
      if (member.computed && isManagedArrayExpression(member.object)) {
        return emitManagedArrayIndexGet(member);
      }
      if (!member.computed && member.property instanceof Identifier) {
        const delegatedPropertyName = (member.property as Identifier).name;
        const receiverClass = classStatementForCppType(cppTypeForExpression(member.object));
        const delegatedProperty = receiverClass
          ? implementedInterfaceProperties(receiverClass).find((candidate) =>
            candidate.property.name.name === delegatedPropertyName &&
            classDelegateReceiverForMember(receiverClass, candidate.property.name.name))
          : undefined;
        if (delegatedProperty) {
          return `${emitExpression(member.object)}->${interfacePropertyGetterName(delegatedPropertyName)}()`;
        }
      }
      if (!isOptionalChainExpression(member.object) && !member.optional && isDynamicValueExpression(member.object) && isClassStoredPropertyMember(member)) {
        ...
        if (receiverType?.endsWith("*")) {
          const receiver = `vexa::toInstance<${receiverType}>(${emitExpression(member.object)})`;
      ...
      if (member.optional && member.computed && !isManagedArrayExpression(member.object)) {
        return `vexa::dynamicGetOptional(vexa::toValue(${emitExpression(member.object)}), vexa::propertyKey(${emitExpression(member.property)}))`;
      }
      const boundMethod: CallableMember | null = memberInfo
        ? classMethodForMember(memberInfo)
        : null;
      const boundClassMethod = boundMethod instanceof ClassMethodMember
      ...
      const enumerableInterface = objectAnalysisType instanceof NamedType &&
        activeInterfaceNames.has(parseTypeNameShape(objectAnalysisType.name).baseName);
      if (member.computed && (enumerableInterface || objectAnalysisType instanceof ObjectType)) {
        return `vexa::enumerableGet(vexa::rawPointer(${emitExpression(member.object)}), vexa::propertyKey(${emitExpression(member.property)}))`;
      }
      ...
      if (!member.computed && (
        ...
        return `vexa::enumerableGet(vexa::rawPointer(${emitExpression(member.object)}), ${cppUtf16String((member.property as Identifier).name)})`;
      }
...
function bindingValueType(binding: BindingName): string {
  ...
  return binding instanceof ObjectBindingPattern ? "vexa::RecordObject*" : "auto";
}
...
function introducedBindingNames(binding: BindingName): string[] {
  ...
  if (binding instanceof ArrayBindingPattern) {
    for (const element of (binding as ArrayBindingPattern).elements) {
      if (!(element instanceof BindingHole)) names.push(...introducedBindingNames(element.name));
    }
    return names;
  }
...
function emitDestructuredBindings(
  ...
  if (binding instanceof Identifier) {
    ...
    if (!sourceType) activeDynamicValueNames.add(binding.name);
    ...
    (binding as ArrayBindingPattern).elements.forEach((element, index) => {
      ...
      if (element.rest) {
        ...
        const initialized = element.initializer
          ? `vexa::destructureDefault(${value}, [&]() { return ${emitExpression(element.initializer)}; })`
          : value;
  ...
  for (const element of objectElements) {
    if (element.rest) {
      emitDestructuredBindings(
        element.name,
        `vexa::recordRest(${source}, {${excludedKeys.map(cppUtf16String).join(", ")}})`,
        lines,
        "vexa::RecordObject*"
      );
      continue;
    ...
    const propertyValue = sourceType === "vexa::Value"
      ...
      : interfaceStatementForCppType(sourceType) !== null
        ? `vexa::enumerableGet(vexa::rawPointer(${source}), ${cppUtf16String(propertyName)})`
        : `vexa::recordGet<vexa::Value>(${source}, ${cppUtf16String(propertyName)})`;
    const value = emitNativeConversion(
      element.initializer
        ? `vexa::destructureDefault(${propertyValue}, [&]() { return ${emitExpression(element.initializer)}; })`
        : propertyValue,
      type
    );
...
function emitTypedArrayDestructuredBindings(
  ...
  for (const element of binding.elements) {
    if (!(element instanceof BindingHole) && (element.rest || !(element.name instanceof Identifier))) {
  ...
  for (const element of binding.elements) {
    ...
    index += 1;
    if (element instanceof BindingHole) continue;
    if (!(element.name instanceof Identifier)) continue;
    ...
    lines.push(`${type} ${cppName(name)} = ${emitNativeConversion(`vexa::arrayGet(vexa::arrayPointer(${source}), ${elementIndex})`, type)}`);
  }
}

function emitDirectMapDestructuredBindings(
  binding: ArrayBindingPattern,
  source: string,
  index: string,
  elementTypes: readonly string[],
  ...
  if (binding.elements.some((element) =>
    !(element instanceof BindingHole) && (element.rest || !(element.name instanceof Identifier)))) {
    return false;
  }
  let elementIndex = 0;
  for (const element of binding.elements) {
    ...
    elementIndex += 1;
    if (element instanceof BindingHole || !(element.name instanceof Identifier)) continue;
    if (currentIndex > 1) return false;
    const type = elementTypes[currentIndex] || "vexa::Value";
    const name = element.name.name;
    activeLocalNames.add(name);
    activeLocalCppTypes.set(name, type);
    if (type === "vexa::Value") activeDynamicValueNames.add(name);
    const arrayType = managedArrayElementType(type);
    if (arrayType !== null) activeGcArrayTypes.set(name, type.slice(0, -1));
    const helper = currentIndex === 0 ? "mapKeyAtAs" : "mapValueAtAs";
    lines.push(`${type} ${cppName(name)} = vexa::${helper}<${type}>(${source}, ${index})`);
  }
  return true;
}

function variableDelegateBackingName(sourceName: string): string {
  return `__vexa_delegate_${cppName(sourceName)}`;
}

function cppVariableDelegateInfo(statement: VarStatement): CppVariableDelegateInfo | null {
  if (!(statement.name instanceof Identifier) || !statement.delegate) return null;
  const delegateType = activeExpressionTypes.get(statement.delegate as Node);
  if (delegateType instanceof FunctionType) {
    return {
      backingName: variableDelegateBackingName(statement.name.name),
      kind: "function",
      valueType: cppTypeForAnalysisType(delegateType.returnType) ?? "vexa::Value",
    };
  }
  if (delegateType instanceof ObjectType) {
    const valueType = delegateType.properties.get("value");
    if (valueType) {
      return {
        backingName: variableDelegateBackingName(statement.name.name),
        kind: "objectValue",
        valueType: cppTypeForAnalysisType(valueType) ?? "vexa::Value",
      };
    }
  }
  return null;
...
function emitVariableDelegateWrite(delegate: CppVariableDelegateInfo, value: string): string {
  if (delegate.kind === "function") {
    return `${delegate.backingName}(${value})`;
  }
  return `vexa::recordSet(${delegate.backingName}, u"value", ${value})`;
}

function emitVariable(statement: VarStatement, forInitializer = false): string {
  if (statement.delegate) {
    throw new CppEmitError("C++ local variable delegates are not implemented yet", statement);
  }
  ...
  const identifierStorageType = statement.initializer instanceof Identifier
    ? storedCppTypeForIdentifier(statement.initializer.name)
    : undefined;
  const requiresDeclaredIdentifierConversion = Boolean(
    declaredCppType &&
    statement.initializer instanceof Identifier &&
    identifierStorageType !== declaredCppType
  );
  const initializer = forInitializer && type !== "auto"
    ? emitNativeConversion(emittedInitializer, type)
    : declaredCppType && (declaredCppType !== initializerType || requiresDeclaredIdentifierConversion) &&
    ...
    ? managedArrayElementType(declaredCppType) !== null &&
        (statement.initializer instanceof CallExpression || statement.initializer instanceof ArrayLiteral)
    ...
    : emittedInitializer;
  const inferredCallDeclaredType = statement.initializer instanceof CallExpression &&
    (statement.initializer as CallExpression).callee instanceof Identifier
    ? activeFunctionStatements.get(((statement.initializer as CallExpression).callee as Identifier).name)?.returnType?.name
    : undefined;
  const inferredCallCppType = statement.initializer instanceof CallExpression
  ...
  let inferredDeclaredTypeName: string | null | undefined = declaredTypeName;
  if (!inferredDeclaredTypeName && typeof inferredCallDeclaredType === "string") {
    inferredDeclaredTypeName = inferredCallDeclaredType;
  }
  if (!inferredDeclaredTypeName) {
    inferredDeclaredTypeName = statement.initializer instanceof CallExpression
      ? declaredCallResultType(statement.initializer)
...
function appendStatementPreamble(lines: string[], statement: Statement, indent: string): void {
  ...
  lines.push(`${indent}VEXA_NATIVE_SOURCE(${file}, ${line}, ${column});`);
}
...
function emitFor(statement: ForStatement, indent: string, label?: string): string {
  ...
  try {
    if (statement.iterationKind || statement.iterator || statement.iterable) {
      ...
      const iteratorBinding = statement.iterator instanceof VarStatement
      ...
      if (statement.iterationKind === "in") {
        ...
        if (!(iteratorBinding instanceof Identifier)) {
      ...
      if (!stringIterable && !binaryIterable && !isArrayExpression(statement.iterable) && !isGeneratorExpression(statement.iterable) && collection !== "map" && collection !== "set" && !deferredNativeArray) {
      ...
      const range = stringIterable
        ? `vexa::stringCharacters(${iterable})`
        : binaryIterable
          ? `*${iterable}`
        : collection === "map"
        ? `*vexa::mapEntries(${nativeCollectionReceiver})`
        : collection === "set"
          ? `*vexa::setValues(${nativeCollectionReceiver})`
          : deferredNativeArray
            ? `vexa::dynamicIterationRange(${iterable})`
      ...
      const tupleTypes = iterableAnalysisType instanceof ArrayType && iterableAnalysisType.elementType instanceof TupleType
        ? iterableAnalysisType.elementType.elements.map(cppArrayElementType).filter((type): type is string => type !== null)
        : null;
      if (collection === "map" && mapTypes && iteratorBinding instanceof ArrayBindingPattern) {
        const source = `__vexa_map_iteration_source_${activeDestructureTemporaryCounter++}`;
        const index = `__vexa_map_iteration_index_${activeDestructureTemporaryCounter++}`;
        if (emitDirectMapDestructuredBindings(iteratorBinding, source, index, mapTypes, bindingLines)) {
          clearExpressionTypeCaches();
          const body = emitStatement(statement.body, `${indent}    `);
          return [
            `${indent}{`,
            `${indent}  auto* ${source} = ${nativeCollectionReceiver};`,
            `${indent}  for (std::size_t ${index} = 0; ${index} < ${source}->size(); ++${index}) {`,
      ...
      if ((mapTypes ?? tupleTypes) && iteratorBinding instanceof ArrayBindingPattern) {
        const arrayBinding = iteratorBinding as ArrayBindingPattern;
        emitTypedArrayDestructuredBindings(arrayBinding, temporary, mapTypes ?? tupleTypes!, bindingLines);
      } else {
    ...
    const initializer = statement.initializer
      ? statement.initializer instanceof VarStatement
...
function emitSwitchBody(statement: SwitchStatement, indent: string): string {
  ...
  statement.cases.forEach((switchCase, index) => {
    ...
    if (!(switchCase.test instanceof StringLiteral)) {
  ...
  if (!statement.catchClause) {
    ...
    try {
      ...
      const nativeBinding = parameter
        ? `auto ${cppName(parameter.name)} = vexa::makeString(vexa::exceptionText(${caughtName}));`
        : "";
...
function callableReturnType(
  ...
  if (!returnType) {
    ...
    if (inferred instanceof FunctionType) {
      const inferredReturn = inferred.returnType;
      if (inferredReturn instanceof NamedType && inferredReturn.name === "Promise") {
        const valueType = namedTypeArgument(inferredReturn, 0);
...
function callableProducesTask(
  ...
  const inferred = activeCallableTypes.get(callableName as Node);
  if (inferred instanceof FunctionType && inferred.returnType instanceof NamedType && inferred.returnType.name === "Promise") {
    return true;
  }
  return Boolean(body && returnExpressions(body).some((expression) =>
    (expression instanceof CallExpression || expression instanceof NewExpression) &&
...
function callableGeneratorInfo(
  ...
  const analyzedReturn = callableType instanceof FunctionType ? callableType.returnType : undefined;
  if (
    analyzedReturn instanceof NamedType &&
    (analyzedReturn.name === "Generator" || analyzedReturn.name === "AsyncGenerator")
  ) {
    const elementType = namedTypeArgument(analyzedReturn, 0);
    return {
...
function callableParameters(
  ...
  borrowedStringParameters: ReadonlySet<string> = new Set()
  ...
  const text = parameters.map((parameter, parameterIndex) => {
    if (!(parameter.name instanceof Identifier)) {
    ...
    const declarationType = borrowedStringParameters.has(sourceName) && type === "std::u16string"
      ? "const std::u16string&"
      : type;
    return `${declarationType} ${cppName(sourceName)}`;
  }).join(", ");
...
function nodeReassignsIdentifier(node: Node, name: string): boolean {
  if (
    node instanceof AssignmentExpression &&
    (node as AssignmentExpression).left instanceof Identifier &&
    ((node as AssignmentExpression).left as Identifier).name === name
  ) {
    return true;
  }
  if (
    node instanceof UpdateExpression &&
    (node as UpdateExpression).argument instanceof Identifier &&
    ((node as UpdateExpression).argument as Identifier).name === name
  ) {
    return true;
  }
  return childNodes(node).some((child) => nodeReassignsIdentifier(child, name));
}
...
function callableSignature(
  ...
  borrowStringParameters = false
): string {
  const resultType = generatorInfo?.resultType ?? callableReturnType(returnType, body, owner, name, taskResult);
  const borrowedStringParameterNames = new Set<string>();
  if (borrowStringParameters && !taskResult && generatorInfo === null) {
    for (const parameter of parameters) {
      const parameterName = callableParameterName(parameter);
      if (parameterName && !nodeReassignsIdentifier(body, parameterName)) {
        borrowedStringParameterNames.add(parameterName);
      }
    }
  }
  const parameterText = callableParameters(
    parameters,
    owner,
    true,
    false,
    borrowedStringParameterNames
  ).text;
...
function withCallableContext<Emit extends () => string>(
  ...
  for (const parameter of parameters) {
    if (parameter.name instanceof Identifier && parameter.typeAnnotation) {
      localDeclaredTypeNames.set((parameter.name as Identifier).name, parameter.typeAnnotation.name);
    }
    if (!(parameter.name instanceof Identifier)) continue;
...
function extensionPropertyCppName(statement: VarStatement, setter = false): string {
  const propertyName = statement.name instanceof Identifier ? statement.name.name : "property";
...
function emitExtensionProperty(statement: VarStatement): string {
  if (!statement.receiverType || !(statement.name instanceof Identifier)) {
...
function emitFunction(statement: FunctionStatement): string {
  ...
  const asyncResultType = !generatorInfo && (statement.async || statement.sync)
    ...
    : null;
  let valueResultType: string;
  if (generatorInfo) {
    valueResultType = generatorInfo.resultType;
  } else if (asyncResultType !== null) {
    valueResultType = asyncResultType;
  } else {
    valueResultType = callableReturnType(statement.returnType, statement.body, statement, statement.name, producesTask);
  }
  const callableResultType = producesTask && asyncResultType === null
    ? `vexa::Task<${valueResultType}>`
    : valueResultType;
  let receiverTypeName: string | null = null;
  if (statement.receiverType) receiverTypeName = statement.receiverType.name;
  let generatorResultType: string | null = null;
  if (generatorInfo) generatorResultType = generatorInfo.resultType;
  return withCallableContext(
    statement.parameters,
    receiverTypeName,
    false,
...
function emitEnum(statement: EnumStatement): string {
  ...
  const type = enumCppType(statement.name.name);
  const lines = [`struct ${cppName(statement.name.name)} final {`];
  statement.members.forEach((member, index) => {
    let value: string;
    if (member.initializer) {
      const raw = emitEnumConstantExpression(member.initializer);
      value = type === "vexa::Value"
        ? `vexa::toValue(${member.initializer instanceof StringLiteral ? `std::u16string(${raw})` : raw})`
        : raw;
    } else if (index === 0) {
      if (type === "std::u16string") {
        throw new CppEmitError("C++ string enums require an initializer for every member", statement);
      }
      value = type === "vexa::Value" ? "vexa::toValue(0.0)" : "0";
    } else {
      const previous = `${cppName(statement.name.name)}::${cppName(statement.members[index - 1]!.name.name)}`;
      if (type === "std::u16string") {
        throw new CppEmitError("C++ string enums require an initializer for every member", statement);
      }
      value = type === "vexa::Value"
        ? `vexa::toValue(vexa::toNativeInt32(${previous}) + 1)`
        : `(${cppName(statement.members[index - 1]!.name.name)} + 1)`;
    }
    const declaration = type === "std::int32_t"
      ? `static constexpr ${type}`
      : `inline static const ${type}`;
    lines.push(`  ${declaration} ${cppName(member.name.name)} = ${value};`);
  });
  lines.push("};");
  return lines.join("\n");
}
...
function emitInterfaceWithActiveTypeParameters(statement: InterfaceStatement): string {
  ...
  const enumerableGetBranches = enumerableProperties.map((property) =>
    `if (__vexa_key == ${cppUtf16String(property.name.name)}) return vexa::toValue(this->${interfacePropertyGetterName(property.name.name)}());`
  );
  const interfaceName = cppName(statement.name.name);
  const qualifiedInterfaceName = statement.typeParameters?.length
    ? `${interfaceName}<${statement.typeParameters.map((parameter) => cppName(parameter.name.name)).join(", ")}>`
    : interfaceName;
  activeDeferredNativeDefinitions.push([
    cppTemplatePrefix(statement.typeParameters),
    `inline vexa::Value ${qualifiedInterfaceName}::enumerableGet(const std::u16string& __vexa_key) { ${enumerableGetBranches.join(" ")} return vexa::Value::undefined(); }`,
  ].join(""));
  return [
    `${cppTemplatePrefix(statement.typeParameters)}class ${interfaceName}${inheritance} {`,
    " public:",
...
function interfaceProperties(
  ...
  for (const member of statement.members) {
    if (member instanceof InterfacePropertyMember) properties.set(member.name.name, member);
  }
...
function interfaceMethods(
  ...
  for (const member of statement.members) {
    if (member instanceof InterfaceMethodMember) methods.set(member.name.name, member);
  }
...
function emitRecordInterfaceAdaptation(expression: Expr, interfaceName: string): string {
  ...
  let result: string;
  try {
    result = `vexa::makeManaged<${recordInterfaceAdapterName(interfaceName)}>(${emitExpression(expression)})`;
  } finally {
...
function emitRecordInterfaceAdapter(statement: InterfaceStatement): string | null {
  ...
  for (const property of interfaceProperties(statement)) {
    ...
    properties.push(`  ${type} ${interfacePropertyGetterName(property.name.name)}() override { return vexa::recordGet<${type}>(record_, ${cppUtf16String(property.name.name)}); }`);
    if (isMutableInterfaceProperty(property)) {
      properties.push(
        `  void ${interfacePropertySetterName(property.name.name)}(${type} value) override { vexa::recordSet(record_, ${cppUtf16String(property.name.name)}, value); }`
      );
    }
  }
  ...
  for (const method of interfaceMethods(statement)) {
    ...
    const dynamicArguments = parameters.names.map((name) =>
      `vexa::toValue(${cppName(name)})`).join(", ");
    const invocation = `vexa::call(vexa::recordGet<vexa::Value>(record_, ${cppUtf16String(method.name.name)}), {${dynamicArguments}})`;
    const body = resultType === "void"
      ? `${invocation};`
      : `return ${emitNativeConversion(invocation, resultType)};`;
    methods.push(`  ${resultType} ${cppName(method.name.name)}(${parameters.text}) override;`);
    activeDeferredNativeDefinitions.push(
      `inline ${resultType} ${adapterName}::${cppName(method.name.name)}(${parameters.text}) { ${body} }`
    );
  }
  return [
    ...
    "};",
    `inline ${interfaceName}* ${interfaceName}::fromRecord(vexa::RecordObject* record) { return vexa::makeManaged<${adapterName}>(record); }`,
  ].join("\n");
}
...
function classPropertyImplementationKind(
  ...
  const constructor = statement.members.find((candidate): candidate is ClassMethodMember =>
    candidate instanceof ClassMethodMember && candidate.name.name === "constructor");
  const constructorProperty = constructor?.parameters.find((parameter) =>
    parameter.name instanceof Identifier && parameter.name.name === propertyName && parameter.accessModifier !== undefined);
  if (constructorProperty) {
    return constructorProperty.accessModifier && !constructorProperty.isReadonly ? 2 : 1;
  }
  let field: ClassFieldMember | null = null;
  for (const member of statement.members) {
    if (member instanceof ClassFieldMember && member.name.name === propertyName) {
  ...
  if (classDelegateReceiverForMember(statement, propertyName)) return 4;
  return classGetterForName(statement, propertyName) ? 3 : 0;
}

function classDelegateReceiverForMember(statement: ClassStatement, memberName: string): string | null {
  for (const delegate of statement.classDelegates ?? []) {
    const interfaceStatement = activeInterfaceStatements.get(parseTypeNameShape(delegate.typeAnnotation.name).baseName);
    if (!interfaceStatement) continue;
    const hasMember = interfaceProperties(interfaceStatement).some((property) => property.name.name === memberName) ||
      interfaceMethods(interfaceStatement).some((method) => method.name.name === memberName);
    if (!hasMember) continue;
    if (delegate.expression instanceof ObjectLiteral) {
      const property = delegate.expression.properties.find((candidate) =>
        candidate instanceof ObjectProperty && candidate.value instanceof Identifier);
      if (property instanceof ObjectProperty && property.value instanceof Identifier) {
        return `this->${cppName(property.value.name)}`;
      }
    }
    if (delegate.expression instanceof Identifier) return `this->${cppName(delegate.expression.name)}`;
  }
  return null;
}
...
function emitInterfacePropertyBridges(statement: ClassStatement): string[] {
  ...
  for (const rawResolvedProperty of implementedInterfaceProperties(statement)) {
    ...
    const delegatedReceiver = implementationKind === 4
      ? classDelegateReceiverForMember(statement, property.name.name)
      : null;
    const getterValue = implementationKind === 3
      ? `this->${propertyName}()`
      : implementationKind === 4 && delegatedReceiver
        ? `${delegatedReceiver}->${interfacePropertyGetterName(property.name.name)}()`
        : `this->${propertyName}`;
    const returnedValue = property.optional
      ? `vexa::toValue(${getterValue})`
      : getterValue;
    const lines = [
    ...
    if (isMutableInterfaceProperty(property)) {
      if (implementationKind === 2) {
        const value = property.optional
          ? emitNativeConversion("__vexa_property_value", implementationType)
          : "__vexa_property_value";
        lines.push(
          `  void ${interfacePropertySetterName(property.name.name)}(${type} __vexa_property_value) override { this->${propertyName} = ${value}; }`
        );
      ...
      } else if (implementationKind === 4 && delegatedReceiver) {
        const value = property.optional
          ? emitNativeConversion("__vexa_property_value", implementationType)
          : "__vexa_property_value";
        lines.push(
          `  void ${interfacePropertySetterName(property.name.name)}(${type} __vexa_property_value) override { ${delegatedReceiver}->${interfacePropertySetterName(property.name.name)}(${value}); }`
        );
      } else if (implementationKind === 3) {
        lines.push(
          `  void ${interfacePropertySetterName(property.name.name)}(${type}) override { throw vexa::runtimeError(${cppUtf16String(`Cannot assign to read-only property '${property.name.name}'`)}); }`
        );
      } else if (implementationKind === 1) {
        lines.push(
          `  void ${interfacePropertySetterName(property.name.name)}(${type}) override { throw vexa::runtimeError(${cppUtf16String(`Cannot assign to read-only property '${property.name.name}'`)}); }`
        );
      } else {
...
function emitClassDelegateMethodBridges(statement: ClassStatement, methods: readonly ClassMethodMember[]): string[] {
  const existingMethods = new Set(methods.map((method) => method.name.name));
  const result: string[] = [];
  for (const delegate of statement.classDelegates ?? []) {
    const interfaceStatement = activeInterfaceStatements.get(parseTypeNameShape(delegate.typeAnnotation.name).baseName);
    if (!interfaceStatement) continue;
    for (const method of interfaceMethods(interfaceStatement)) {
      if (existingMethods.has(method.name.name)) continue;
      const receiver = classDelegateReceiverForMember(statement, method.name.name);
      if (!receiver) continue;
      const resultType = method.returnType ? cppTypeForDeclaredName(method.returnType.name) : "void";
      if (!resultType || resultType === "void" && method.returnType) {
        throw new CppEmitError(`C++ cannot map delegated method '${method.name.name}'`, statement);
      }
      const parameters = callableParameters(method.parameters, statement, false).text;
      const argumentsText = method.parameters.map((parameter) =>
        parameter.name instanceof Identifier ? cppName(parameter.name.name) : "").join(", ");
      const invocation = `${receiver}->${cppName(method.name.name)}(${argumentsText})`;
      result.push(`  ${resultType} ${cppName(method.name.name)}(${parameters}) override { ${resultType === "void" ? `${invocation};` : `return ${invocation};`} }`);
      existingMethods.add(method.name.name);
    }
  }
  return result;
}
...
function emitClassMethodWithActiveTypeParameters(
  ...
  if (generatorInfo) {
    ...
    valueResultType = callableReturnType(method.returnType, method.body, statement, method.name, producesTask);
  }
  const callableResultType = producesTask && asyncResultType === null
    ? `vexa::Task<${valueResultType}>`
    : valueResultType;
...
function emitForeignLibraryCppClass(statement: ClassStatement): string {
  ...
  for (const method of methods) {
    ...
    const result = returnDefinition.async
      ? returnTypeName === "FFIPointer"
        ? `    return vexa::runAsyncMapped(${asyncInvocation}, [](void* value) { return vexa::makeManaged<vexa::FFIPointerObject>(value); });`
        : returnTypeName === "boolean"
          ? `    return vexa::runAsyncMapped(${asyncInvocation}, [](std::uint8_t value) { return value != 0; });`
          : `    return vexa::runAsync(${asyncInvocation});`
      : returnTypeName === "void"
        ? `    ${invocation};`
        : returnTypeName === "boolean"
          ? `    return ${invocation} != 0;`
          : returnTypeName === "FFIPointer"
            ? `    return vexa::makeManaged<vexa::FFIPointerObject>(${invocation});`
            : `    return ${invocation};`;
    methodLines.push([
...
function emitForeignStructCppClass(statement: ClassStatement): string {
  ...
  return [
    ...
    `      : buffer_(vexa::makeManaged<vexa::ArrayBufferObject>(${definition.size}))${referenceInitializers.length ? `, ${referenceInitializers.join(", ")}` : ""} { ${assignments} }`,
...
function emitClass(statement: ClassStatement): string {
  if (statement.declared && (statement.name.name === "TextEncoder" || statement.name.__vexaNativeOriginalName === "TextEncoder")) return "";
...
function emitClassWithActiveTypeParameters(statement: ClassStatement): string {
  ...
  if (
    ...
    statement.members.some((member) => !(member instanceof ClassMethodMember) && !(member instanceof ClassFieldMember))
  ) {
    const unsupportedMembers = statement.members
      .filter((member) => !(member instanceof ClassMethodMember) && !(member instanceof ClassFieldMember))
  ...
  for (const rawMember of statement.members) {
    if (!(rawMember instanceof ClassFieldMember)) continue;
  ...
  if (constructorMethod) {
    const superStatement = constructorMethod.body.body.find((candidate) => {
      ...
      return expression instanceof CallExpression && identifierName((expression as CallExpression).callee) === "super";
    });
      ...
      () => {
        ...
        for (const rawProperty of typedConstructorProperties) {
          ...
          const initializer = property.type === "vexa::Value" && parameterType?.endsWith("*")
            ? `(${property.name} ? vexa::toValue(${property.name}) : vexa::Value::undefined())`
            : property.name;
  ...
  for (const rawMember of statement.members) {
    if (rawMember instanceof ClassMethodMember && rawMember.name.name !== "constructor") {
  ...
  const delegatedMethodBridges = emitClassDelegateMethodBridges(statement, methods);
  if (delegatedMethodBridges.length > 0) {
    if (activeAccess !== "public") {
      methodLines.push(" public:");
      activeAccess = "public";
    }
    methodLines.push(...delegatedMethodBridges);
  }
  for (const method of methods) {
  ...
  const addDynamicProperty = (name: string, valueType: string, immutable: boolean): void => {
    ...
    dynamicPropertyReads.push({
      key: name,
      body: `return vexa::toValue(${storedValue});`,
    });
    if (!immutable) {
      dynamicPropertyWrites.push({
        key: name,
        body: `${name} = ${emitNativeConversion("__vexa_value", valueType)}; return __vexa_value;`,
      });
    }
  };
  ...
  for (const method of methods) {
    if (
      ...
      method.parameters.some((parameter) => !(parameter.name instanceof Identifier) || parameter.rest)
    ) {
      continue;
    }
...
function emitLabeledStatement(statement: LabeledStatement, indent: string): string {
  ...
  if (statement.body instanceof ForStatement) {
    return emitFor(statement.body as ForStatement, indent, label);
  }
  if (statement.body instanceof WhileStatement) {
  ...
  if (statement.body instanceof DoWhileStatement) {
...
function emitStatement(statement: Statement, indent = ""): string {
  switch (statement.kind) {
    ...
    case NodeKind.ExprStatement: {
      ...
      if (expression instanceof UnaryExpression && (expression as UnaryExpression).operator === "yield*") {
        ...
        return [
          ...
          `${indent}  co_yield ${emitNativeConversion(temporary, activeGeneratorResultType)};`,
          `${indent}}`,
        ].join("\n");
      }
    ...
    case NodeKind.IfStatement: {
      ...
      const previousInstanceofTemporaries = activeInstanceofTemporaryExpressions;
      activeInstanceofTemporaryExpressions = new Map(activeInstanceofTemporaryExpressions);
      const instanceofTemporaryNames: string[] = [];
      for (const narrowing of narrowings) {
        const temporary = `__vexa_instanceof_${activeInstanceofTemporaryCounter++}`;
        activeInstanceofTemporaryExpressions.set(
          instanceofNarrowingKey(narrowing.sourceName, narrowing.targetName),
          temporary
        );
        instanceofTemporaryNames.push(temporary);
      }
      ...
      if (narrowings.length > 0) {
        thenBody = injectBlockPreamble(thenBody, narrowings.flatMap((narrowing, index) => {
          const temporary = instanceofTemporaryNames[index]!;
          return [
            `${indent}  auto* ${cppName(narrowing.sourceName)} = ${temporary};`,
          ];
        }));
      }
      const condition = emitParenthesizedCondition(branch.condition);
      activeInstanceofTemporaryExpressions = previousInstanceofTemporaries;
      const alternate = branch.elseBranch ? ` else ${emitBody(branch.elseBranch, indent)}` : "";
      const temporaryDeclarations = narrowings.map((narrowing, index) =>
        `${indent}auto* ${instanceofTemporaryNames[index]!} = vexa::toInstanceOrNull<${narrowing.targetType}>(${cppName(narrowing.sourceName)});`
      );
      return `${temporaryDeclarations.length > 0 ? `${temporaryDeclarations.join("\n")}\n` : ""}${indent}if ${condition} ${thenBody}${alternate}`;
    }
    case NodeKind.WhileStatement: {
    ...
    case NodeKind.ReturnStatement: {
      ...
      if (activeFinallyProtectedDepth > 0 && activeCallableResultType) {
        ...
        if (activeCppTypeParameters.has(activeCallableResultType)) {
          return `${indent}vexa::throwReturn<${activeCallableResultType}>([&]() -> decltype(auto) { return ${emitExpression(returned)}; });`;
        }
        ...
        const flattened = activeAsyncResultType && returnedType instanceof NamedType && returnedType.name === "Promise"
          ? `(co_await ${emitted})`
          : emitted;
...
export type NativeCollectionRepresentation = "specialized" | "generic";
...
function buildCppProgram(
  ...
  for (const bodyStatement of program.body) {
    const statement = bodyStatement instanceof ExportStatement && (bodyStatement as ExportStatement).declaration
      ? (bodyStatement as ExportStatement).declaration!
      : bodyStatement;
    statements.push(statement);
    if (statement instanceof InterfaceStatement) interfaces.push(statement as InterfaceStatement);
    else if (statement instanceof EnumStatement) enums.push(statement as EnumStatement);
    else if (statement instanceof TypeAliasStatement) typeAliases.push(statement as TypeAliasStatement);
    else if (statement instanceof ClassStatement) classes.push(statement as ClassStatement);
    else if (statement instanceof FunctionStatement) functions.push(statement as FunctionStatement);
    else if (statement instanceof VarStatement && (statement as VarStatement).receiverType) {
  ...
  while (pendingLiteralNodes.length > 0) {
    ...
    if (node instanceof UnaryExpression && usesPooledFunctionTypeof(node as UnaryExpression)) {
  ...
  const enumCppTypes = new Map<string, "std::int32_t" | "std::u16string" | "vexa::Value">();
  for (const statement of enums) {
    enumNames.add(statement.name.name);
    const hasStringMember = statement.members.some((member) => member.initializer instanceof StringLiteral);
    const hasNonStringMember = statement.members.some((member) =>
      member.initializer !== undefined && !(member.initializer instanceof StringLiteral));
    enumCppTypes.set(
      statement.name.name,
      hasStringMember
        ? hasNonStringMember ? "vexa::Value" : "std::u16string"
        : "std::int32_t"
    );
  }
  activeEnumNames = enumNames;
  activeEnumCppTypes = enumCppTypes;
  const enumStatements = new Map<string, EnumStatement>();
  for (const statement of enums) {
    enumStatements.set(statement.name.name, statement);
  }
  activeEnumStatements = enumStatements;
  ...
  for (const statement of extensionProperties) {
    if (statement.name instanceof Identifier && statement.receiverType) {
  ...
  activeInstanceofTemporaryExpressions = new Map();
  activeSharedBindingNames = new Set();
  activeSharedBindingCandidates = new Set();
  ...
  const variableDelegates = new Map<string, CppVariableDelegateInfo>();
  for (const statement of statements) {
    const delegate = statement instanceof VarStatement ? cppVariableDelegateInfo(statement) : null;
    if (delegate && statement instanceof VarStatement && statement.name instanceof Identifier) {
      variableDelegates.set(statement.name.name, delegate);
    }
  }
  activeVariableDelegates = variableDelegates;
  activeImplicitReceiverIdentifiers = semantics.implicitReceiverIdentifiers ?? new Set<Node>();
  ...
  activeAutoAwaitExpressions = semantics.autoAwaitExpressions ?? new Set<Node>();
  ...
  activeNativeCollectionRepresentation = semantics.nativeCollectionRepresentation ?? "specialized";
  activeExpectedLambdaResultCppType = null;
  activeExpectedLambdaParameterCppTypes = null;
  const operatorMethodsByNameNode = new Map<Node, ClassMethodMember>();
  for (const statement of classes) {
    for (const member of statement.members) {
      if (member instanceof ClassMethodMember && (member as ClassMethodMember).operator) {
  ...
  for (const statement of topLevelVariables) {
    ...
    const analyzedInferredType = statement.initializer
      ? cppTypeForExpression(statement.initializer)
      : null;
    const inferredType = statement.initializer instanceof ArrayLiteral
  ...
  for (const info of topLevelVariableInfo) {
    if (info.pointee && info.statement.name instanceof Identifier) {
  ...
  activeRuntimeName = "runtime";
  for (const info of topLevelVariableInfo) {
    if (!(info.statement.name instanceof Identifier)) continue;
  ...
  for (const statement of statements) {
    if (
      ...
      (statement instanceof VarStatement && Boolean((statement as VarStatement).receiverType))
    ) continue;
    if (statement instanceof VarStatement) {
      const variable = statement as VarStatement;
      if (variable.delegate && variable.name instanceof Identifier) {
        const delegate = activeVariableDelegates.get(variable.name.name);
        if (!delegate) {
