...
function cppArrayElementType(type: AnalysisType): string | null {
  if (type.kind === "builtin" && type.name === "string") return "vexa::Text";
  if (type.kind === "literal" && type.base === "string") return "vexa::Text";
...
function emitConvertedValue(expression: Expr, resultType: string): string {
  ...
  if (resultType === "vexa::Value" &&
      (sourceType?.kind === "function" || expression.kind === NodeKind.ArrowFunctionExpression || expression.kind === NodeKind.FunctionExpression) &&
    ...
    const callableExpression = expression.kind === NodeKind.ArrowFunctionExpression || expression.kind === NodeKind.FunctionExpression
      ...
      : null;
    const functionType = sourceType?.kind === "function" ? sourceType as FunctionType : null;
...
function emitPropertyAssignment(
  ...
  if (assignment.operator === "=") {
    ...
    const assignedValue = expectedType
      ...
      : (property.kind === "dynamic" || property.kind === "record") &&
          (activeExpressionTypes.get(assignment.right as Node)?.kind === "function" ||
    ...
    return `([&]() { ${emitNativePropertyReceiverDeclaration(property, receiver)}${keyDeclaration} auto __vexa_property_value = ${assignedValue}; ${emitNativePropertySet(property, "__vexa_property_receiver", "__vexa_property_value", key)}; return __vexa_property_value; }())`;
  }
  ...
  const value = !activeHasExpressionTypes || property.kind === "dynamic" || property.kind === "record" ||
    ...
    : `(__vexa_property_current ${binaryOperator} __vexa_property_operand)`;
  return `([&]() { ${emitNativePropertyReceiverDeclaration(property, receiver)}${keyDeclaration} auto __vexa_property_current = ${emitNativePropertyGet(property, "__vexa_property_receiver", key)}; auto __vexa_property_operand = ${emitExpression(assignment.right)}; auto __vexa_property_value = ${value}; ${emitNativePropertySet(property, "__vexa_property_receiver", "__vexa_property_value", key)}; return __vexa_property_value; }())`;
}
...
function emitPropertyUpdate(
  ...
  return `([&]() { ${emitNativePropertyReceiverDeclaration(property, receiver)}${keyDeclaration} auto __vexa_property_current = ${emitNativePropertyGet(property, "__vexa_property_receiver", key)}; auto __vexa_property_value = ${updated}; ${emitNativePropertySet(property, "__vexa_property_receiver", "__vexa_property_value", key)}; return ${returned}; }())`;
}
...
function methodTemplateBindings(
  ...
  parameters.forEach((parameter, index) => {
    ...
    if (callbackParameter && !bindings.has(callbackParameter.name.name)) {
      if (analyzedArgumentType?.kind === "function") {
...
function emitCall(call: CallExpression): string {
  ...
  if (calleeName === "nativeStatPath" || calleeName === "nativeReadDirectory") {
    ...
    return `vexa::${calleeName}(${activeRuntimeName}, vexa::convertValue<std::string>(${emitExpression(call.args[0]!)}))`;
  }
  if (calleeName === "nativeCreateDirectory" || calleeName === "nativeRemovePath") {
    ...
    return `vexa::${calleeName}(${activeRuntimeName}, vexa::convertValue<std::string>(${emitExpression(call.args[0]!)}), vexa::convertValue<bool>(${emitExpression(call.args[1]!)}))`;
  }
...
function callableProducesTask(
  ...
  const inferred = activeCallableTypes.get(callableName as Node);
  if (inferred?.kind === "function" && inferred.returnType.kind === "named" && inferred.returnType.name === "Promise") {
    return true;
  }
...
function emitExtensionProperty(statement: VarStatement): string {
  ...
  return withCppTypeParameters<string>(statement.typeParameters, () => {
    ...
    if (statement.initializer) {
      ...
      () => {
        ...
        try {
          return `${cppTemplatePrefix(statement.typeParameters)}${resultType} ${extensionPropertyCppName(statement)}(${receiverType} __vexa_extension_self) { return ${emitConvertedValue(statement.initializer!, resultType)}; }`;
        } finally {
          activeThisExpression = previousThisExpression;
        }
      }
    ...
    for (const accessor of statement.accessors!) {
      ...
      emittedAccessors.push(withCallableContext<string>(
        accessor.parameters,
        statement.receiverType!.name,
        false,
        null,
...
function emitInterface(statement: InterfaceStatement): string {
  return withCppTypeParameters<string>(statement.typeParameters, () => emitInterfaceWithActiveTypeParameters(statement));
}
...
function emitClassMethod(
  ...
  return withCppTypeParameters<string>(method.typeParameters, () =>
...
function emitClass(statement: ClassStatement): string {
  return withCppTypeParameters<string>(statement.typeParameters, () => emitClassWithActiveTypeParameters(statement));
}
...
function emitClassWithActiveTypeParameters(statement: ClassStatement): string {
  ...
  if (constructorMethod) {
    ...
    constructor = withCallableContext<string>(
