...
describe("explicit return type quick fixes", () => {
  it("adds an inferred return type to a function declaration", () => {
    const source = "function add(a: number, b: number) {\n  return a + b\n}\n";
    const session = createAnalysisSession(source);
    const actions = createReturnTypeCodeActions({
      uri: URI,
      ast: session.ast,
  ...
  it("offers the fix while the cursor is inside the parameter list", () => {
    const source = "function add(a: number, b: number) {\n  return a + b\n}\n";
    const session = createAnalysisSession(source);
    const actions = createReturnTypeCodeActions({
      uri: URI,
  ...
  it("wraps the inferred type in Promise for async functions", () => {
  ...
  it("adds a return type to a class method", () => {
  ...
  it("does not offer the fix when a return type already exists", () => {
  ...
  it("does not offer the fix when the return type cannot be inferred", () => {
  ...
  it("does not offer the fix outside the signature header", () => {
    const source = "function add(a: number, b: number) {\n  return a + b\n}\n";
    const session = createAnalysisSession(source);
    const actions = createReturnTypeCodeActions({
      uri: URI,
