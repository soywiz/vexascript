import { FunctionType } from "../analysis/types";
import { Identifier } from "compiler/ast/ast";
import type { MemberExpression, Statement } from "compiler/ast/ast";
...
export function findAmbientImportedOverloadRange(
  ...
  for (const declaration of overloadDeclarations) {
    ...
    if (!(declarationType instanceof FunctionType)) {
      continue;
    }
...
export async function resolveAmbientModuleObjectMemberDefinition(
  ...
  if (!(memberExpression.object instanceof Identifier) || !context.session.ast) {
    return null;
  }
