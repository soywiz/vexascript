...
`;
  ...
  it("narrows contradictory literal-union conjunctions to never", () => {
    const source = dedent`
      fun handle(kind: "ok" | "error") {
  ...
  it("infers array comprehension element types", () => {
    ...
    expect(symbolsOfVisibleSymbolsAt(source, 1, 6).get("rangeValues")?.valueType).toBe("int[]");
  });
  ...
  it("infers Array.from element types from boxed strings", () => {
    const source = dedent`
      const values = Array.from(new String("Ada"))
    `;
    const analysis = new Analysis(parseFile(tokenizeReader(source)));
  ...
  it("infers Array.from element types from typed arrays", () => {
    ...
    expect(analysis.getIssues().map((issue) => issue.message)).toEqual([]);
    expect(symbolsOfVisibleSymbolsAt(source, 0, 6).get("values")?.valueType).toBe("number[]");
  });

  it("infers Array.from mapper callback results", () => {
    const source = dedent`
      const values = Array.from([1], (_value, index) => index.toString())
    `;
    const analysis = new Analysis(parseFile(tokenizeReader(source)));
  ...
  it("infers Promise.all values from strings", () => {
    const source = dedent`
      const values = Promise.all("Ada")
    `;
    const analysis = new Analysis(parseFile(tokenizeReader(source)));
