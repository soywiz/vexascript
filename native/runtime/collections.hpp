...
class MapObject final : public cppgc::GarbageCollected<MapObject<K, V>>, public MapLikeObject {
 ...
 public:
  ...
  K keyAt(std::size_t index) const {
    if (dynamic_backing_) return convertValue<K>(dynamic_backing_->dynamicMapKeyAt(index));
    return index < storage_->entries.size() ? storage_->entries[index].key.load() : K{};
  }

  V valueAt(std::size_t index) const {
    if (dynamic_backing_) return convertValue<V>(dynamic_backing_->dynamicMapValueAt(index));
    return index < storage_->entries.size() ? storage_->entries[index].value.load() : V{};
  }
