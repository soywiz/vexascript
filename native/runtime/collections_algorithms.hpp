...
template <typename Result, typename K, typename V>
inline Result mapKeyAtAs(const MapObject<K, V>* map, std::size_t index) {
  return convertValue<Result>(map->keyAt(index));
}

template <typename Result, typename K, typename V>
inline Result mapValueAtAs(const MapObject<K, V>* map, std::size_t index) {
  return convertValue<Result>(map->valueAt(index));
}
