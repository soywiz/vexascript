...
BigInt::BigInt(std::string_view text) { parse(text); }
BigInt::BigInt(std::u16string_view text) { parse(text); }

bool BigInt::isZero() const { return limbs_.empty(); }
bool BigInt::isNegative() const { return negative_; }
bool BigInt::isOdd() const { return !limbs_.empty() && (limbs_[0] & 1U) != 0; }
...
std::uint64_t BigInt::toUint64Modulo() const {
...
BigInt& BigInt::operator&=(const BigInt& other) { return applyBitwise(other, '&'); }
BigInt& BigInt::operator|=(const BigInt& other) { return applyBitwise(other, '|'); }
BigInt& BigInt::operator^=(const BigInt& other) { return applyBitwise(other, '^'); }
...
BigInt BigInt::operator++(int) { BigInt previous = *this; ++*this; return previous; }
BigInt& BigInt::operator--() { return *this -= BigInt(1); }
BigInt BigInt::operator--(int) { BigInt previous = *this; --*this; return previous; }
...
std::strong_ordering operator<=>(const BigInt& left, const BigInt& right) {
...
void BigInt::assignSigned(Signed value) {
...
void BigInt::assignUnsigned(Unsigned value) {
  while (value != 0) {
...
void BigInt::parse(std::basic_string_view<Character> text) {
...
void BigInt::multiplySmall(std::uint32_t multiplier) {
  std::uint64_t carry = 0;
...
void BigInt::addSmall(std::uint32_t value) {
...
std::uint32_t BigInt::divideSmall(std::uint32_t divisor) {
...
void BigInt::setBit(std::size_t index) {
...
std::pair<BigInt, BigInt> BigInt::divideAndRemainder(
...
std::size_t BigInt::toShiftCount() const {
...
void BigInt::shiftLeft(std::size_t amount) {
...
void BigInt::shiftRight(std::size_t amount) {
  if (isZero() || amount == 0) return;
  const bool wasNegative = negative_;
  bool discarded = false;
  const std::size_t whole = amount / 32;
  const unsigned partial = amount % 32;
  for (std::size_t index = 0; index < std::min(whole, limbs_.size()); ++index) {
    discarded |= limbs_[index] != 0;
  }
...
std::vector<std::uint32_t> BigInt::twosComplement(std::size_t width) const {
...
BigInt BigInt::fromTwosComplement(std::vector<std::uint32_t> bits) {
  BigInt result;
...
BigInt& BigInt::applyBitwise(const BigInt& other, char operation) {
...
BigInt remainder(BigInt left, const BigInt& right) { return left %= right; }
bool Boolean(const BigInt& value) { return !value.isZero(); }
