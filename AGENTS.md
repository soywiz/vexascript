...
- Why this matters (the point of unification work): parallel branches of the same logic drift over time. A bug fixed in one branch stays broken in the others, and every new case has to be re-handled in each copy. This divergence is the root cause of recurring bugs in this codebase (see the Zod/Pixi/`docs/journal/` regressions where a feature looked fixed in one surface but stayed broken in another). Therefore, collapsing N copies of a behavior into one shared path is **high value even when it does not reduce line count** — it removes a whole class of future divergence bugs. Do not dismiss a duplicate-collapsing change as "marginal" just because the diff is roughly neutral; the value is in there being one place to fix, not in the line delta.
...
- Treat net code growth during cleanup work as a warning sign — but scope this correctly. The concern is *additive* unification: adding a shared abstraction while leaving the old branches in place (net growth = incomplete unification). It does NOT apply to collapsing existing duplicates into one path, which is valuable even at neutral or slightly positive line count (e.g. one shared helper plus its doc comment replacing several near-identical copies). If a change adds significantly more code than it removes, justify why the added structure is temporary and identify which old paths should be deleted next.
...
- Editor semantics must be type-driven. Do not hardcode framework/domain value catalogs, synthesize literal unions that are absent from the declarations, or choose hover/definition targets through package-wide same-name searches. Completion, hover, diagnostics, and navigation must derive from the parsed TypeScript/VexaScript declarations and their resolved aliases, generics, mapped types, inheritance, and declaration provenance. When typings intentionally expose an open type such as `string`, return no finite literal-value completion rather than inventing suggestions.
...
- From now on, every journal entry must include an `Execution metadata` section identifying the AI model and provider used for the task, plus a `Prompts` section that records the user prompt or prompts that explicitly requested the journal work or materially defined what the journal entry must capture. Obtain the exact active model and provider automatically from the execution context or runtime metadata; never ask the user to supply them. If the runtime does not expose an exact model identifier, record `Unavailable (not exposed by runtime)` rather than guessing or shifting the burden to the user. Preserve prompts verbatim, in their original language, and in chronological order so the entry retains its original intent and provenance. Do not translate prompts. Surrounding journal prose must remain in English.
