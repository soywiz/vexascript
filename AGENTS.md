...
- LSP server and features:
  ...
  - Auto-await gutter decorations (lines with an implicit `await` inside `sync` functions, served via the custom `mylang/autoAwaitDecorations` request and the Monaco glyph margin): `compiler/lsp/autoAwaitDecorations.ts`
