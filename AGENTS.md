...
- From now on, every journal entry must include an `Execution metadata` section identifying the AI model and provider used for the task. Obtain the exact active model and provider automatically from the execution context or runtime metadata; never ask the user to supply them. If the runtime does not expose an exact model identifier, record `Unavailable (not exposed by runtime)` rather than guessing or shifting the burden to the user. Surrounding journal prose must remain in English.
