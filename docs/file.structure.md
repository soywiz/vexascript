...
- Embedded runtime declarations:
  ...
  - Single browser-safe `ensureCompilerRuntimePrograms()` that loads both runtime declaration programs (ECMAScript + VexaScript); called once by each async entry point (CLI via `cli/cliShared.ts`, LSP server in its `initialize` handler, MCP server at startup, tests via `compiler/test/expect.ts`) so synchronous getters are ready without an import-time top-level await: `compiler/runtime/ensureRuntimePrograms.ts`
