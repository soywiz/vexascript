...
- Native C++ support:
  - Cached Oilpan runtime under `native/runtime/`: `runtime.hpp` is the generated-code umbrella; focused headers isolate arrays, collections, dates, strings, regular expressions, Intl, binary data, promises, JSON, objects, tasks, math, console, and supporting layers; `runtime.cpp`, `bigint.cpp`, and `utf.cpp` form the cached static library while `bigint.hpp` and `utf.hpp` contain their declarations. The runtime provides reference-semantic traced `ArrayObject<T>` storage, a native `performance.now()` steady clock, dependency-free arbitrary-precision integers, and UTF-16/UTF-8 operating-system boundary adapters. Vendored standalone Oilpan (forked 20260622) and trimmed mimalloc source archives remain at `native/oilpan-20260622.zip` and `native/mimalloc-3.4.3.zip`.
