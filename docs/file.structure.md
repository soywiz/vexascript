...
- Canonical source spelling configuration and defaults shared by project loading and editor quick fixes: `compiler/canonicalSyntax.ts`
- Emitter / transpilation:
  ...
  - Browser-compatible C++ emission backend, sharing the normal parse and analysis pipeline; `compiler/runtime/nativeStandardLibraryCoverage.ts` locks every named ECMAScript runtime family to an executable case or an explicit unsupported diagnostic. Native behavior is validated by the compiled executable smoke: `compiler/runtime/cppEmitter.ts`, smoke test: `tests/native/nativeSmoke.native.ts`, fixture: `samples/native-language-smoke/` (`language-features.vx` aggregates focused compiler-language cases under `language-features/`, while `standard-library.vx` aggregates the declaration-locked runtime families under `standard-library/`), documented technical gaps: `docs/native-missing-runtime.md`.
  ...
  - Native dependency-ordered module-graph compilation, reusing the shared import resolver and merging local module ASTs for one whole-program analysis before emitting a shared header, an entry point, and one C++ translation unit per source module; explicit `?text` imports become compile-time string literals and remain watched inputs: `compiler/runtime/nativeModuleGraph.ts`, `compiler/runtime/cppEmitter.ts`, tests: `tests/native/nativeModuleGraph.native.ts`
...
- Runnable samples and sample-test harness: `samples/`, `samples/samples.test.ts`
  - Each sample directory can be discovered as a runnable console sample with `expected.txt`, a configured bundle sample with a sample-local VexaScript config entrypoint, or both; the harness runs `main.vx` with `runFile` for console samples and independently validates every configured bundle entrypoint, including browser samples that also ship deterministic console coverage.
  ...
  - Sample-local VexaScript config files are loaded by `compiler/project.ts` through either the active or an explicitly supplied VFS, with JSON caches isolated per VFS instance; configs can set canonical immutable/function declaration spellings through `canonicalSyntax`, JSX factories/import sources (including collision-safe automatic Preact `h`/`Fragment` bindings for JSX-containing modules), `compilerOptions.lib` entries such as `dom` for DOM ambient declarations, `compilerOptions.types` entries such as `node` for loading ambient declarations from runtime packages or `@types` packages, `outDir` build output directories for static site generation, and `serveMappings` aliases that expose files or directories under different served paths. TypeScript config files remain a fallback for compatibility.
  - Preact browser sample: `samples/preact/` validates context providers and consumers, generic hook typing across `preact` and `preact/hooks`, memoized context values, shared state across multiple component instances, and browser rendering through `html.vx`.
  ...
  - Ecosystem TypeScript stress samples use unmodified published packages with deterministic runtime output: `samples/zustand/` covers curried store factories and state callbacks; `samples/hono/` covers typed web routing plus a real Node HTTP listener; `samples/rxjs/` covers overloaded operator composition; `samples/xstate/` covers state-machine configuration inference; `samples/tanstack-table/` covers recursive column and row generics; `samples/effect/` covers namespace barrels and higher-kinded conditional types; `samples/drizzle/` and `samples/kysely/` cover fluent SQL builders and mapped row shapes; `samples/trpc/` covers recursive routers and procedure inference; `samples/viem/` covers const tuples, conditional validators, and ABI-derived types; `samples/react-hook-form/` covers recursive form-state types, utility aliases, and overloaded call-signature objects; `samples/d3/` combines generic data APIs with a real SVG browser chart; `samples/commander/` covers fluent CLI builders and typed actions; and `samples/codemirror/` covers recursive extension types, transactions, and a real browser editor.
  ...
  - Syntax tour sample: `samples/syntax-tour/` is a multi-file runnable fixture that exercises the broad supported VexaScript syntax surface, canonical `const`/`func` plus legacy `val`/`fun` and short `fn` aliases, canonical syntax and JSX factory configuration, local imports, declarations/types, classes, extensions/operators, index operator overloads, property references, delegates, control flow, and async `sync` output.
  ...
  - Native language smoke sample: `samples/native-language-smoke/` is the multi-file end-to-end native fixture. Focused language semantics live under `language-features/`, and ECMAScript runtime families live under `standard-library/`. Both the JavaScript sample harness and `tests/native/nativeSmoke.native.ts` compare observable output with the single shared `expected.txt`; the native test invokes the public `cpp link` command and runs the linked Oilpan binary.
...
- CLI (`cli/`):
  ...
  - `cpp` and `cpp build` compile `.vx` and `.ts` entrypoints to one C++ translation unit by default; `--module-files` emits a shared `program.hpp`, `main.cpp`, and deterministic `module-XXXX.cpp` files instead. `cpp link` and `cpp run` reuse the same module graph, cache generated C++ and native executables under `<input>.build/`, and link with Oilpan. All native forms print their generation phase timings.
...
- Native C++ support:
  - Cached Oilpan runtime under `native/runtime/`: `runtime.hpp` is the generated-code umbrella; focused `.hpp`/`.cpp` pairs isolate arrays, collections, dates, strings, regular expressions, Intl, binary data, promises, JSON, objects, tasks, math, console, and supporting layers. Every concrete implementation is compiled independently into the cached static library; only templates and `constexpr` implementations remain in headers. ECMAScript Map/Set/WeakMap/WeakSet use concrete `Value`-backed runtime classes rather than one C++ class per source type argument. Language arrays retain typed adapters in this phase, with compatible covariant views sharing dynamic backing storage; native-only task arrays retain typed slots. Removing the remaining `ArrayObject<T>` adapter and validating the final `any[]` contract is tracked as the next runtime-erasure phase. The runtime also provides a native `performance.now()` steady clock, dependency-free arbitrary-precision integers, and UTF-16/UTF-8 operating-system boundary adapters. Vendored standalone Oilpan (forked 20260622) and trimmed mimalloc source archives remain at `native/oilpan-20260622.zip` and `native/mimalloc-3.4.3.zip`.
  - Native CLI adapters for filesystem/process access, in-process JavaScript bundling, and self-hosted C++ linking: `native/modules/cliShared.ts` composes the native local-module graph (`native/modules/moduleGraph.ts`), package declaration resolver (`native/modules/nodeModuleImportResolution.ts`), declaration sources loaded directly through `?text` (`native/modules/domDeclarations.ts`, `native/modules/runtimeDeclarationSources.ts`), native built-in module metadata (`native/modules/nodeModule.ts`), and the shared token-based package bundler (`cli/nodeModuleBundle.ts`); `native/modules/cliIo.ts` and `native/modules/nodeFsPromises.ts` provide native host I/O, including byte-preserving file reads used for runtime fingerprints, while `native/modules/nativeBuild.ts` prepares Oilpan/mimalloc and builds runtime objects in temporary storage before retaining only the content-addressed `.a` archive and compatible Clang PCH
  - Native backend usage, requirements, supported surface, and module limitations: `docs/native.md`; intentionally unavailable ECMAScript/host APIs, compact English-only `Intl` scope, and technical reasons: `docs/native-missing-runtime.md`
...
- Monaco editor support for the website embeds (project folder: `website/src/assets/monaco/`):
  ...
  - Dedicated browser language worker that owns parsing, analysis, diagnostics, navigation, semantic tokens, code actions, inlay hints, and formatting so editor-triggered compiler work stays off the UI thread: `website/src/assets/monaco/vexaLanguageWorker.ts`
  - Worker request client that preserves valid empty/null language results separately from transport failures, preventing accidental main-thread retries: `website/src/assets/monaco/languageWorkerClient.ts`
  - Cached analysis-session resolver used inside the language worker for imported workspace files: `website/src/assets/monaco/workspaceSessions.ts`
...
- LSP server and features:
  ...
  - Shared, package-independent declaration-graph primitives for .d.ts files (source/program parsing with per-file caches, relative-specifier resolution, and the `/// <reference path>` extractor), reused by both the node_modules typings loader and the ambient types loader so the two do not diverge; see the generalize-dts-declaration-loading task doc for the planned full general loader: `compiler/lsp/dtsModuleGraph.ts`
  ...
  - Diagnostics and shared LSP diagnostic-tag constants reused by both the Node server and browser Monaco/embed paths: `compiler/lsp/diagnostics.ts`, `compiler/lsp/diagnosticTags.ts`
...
- Website and embeddable learning playground (project root: `website/`):
  - 11ty configuration and static-site build surface, including the dedicated VexaScript project-configuration reference: `website/eleventy.config.mjs`, `website/src/index.njk`, `website/src/quickstart.njk`, `website/src/config.njk`, `website/src/samples.njk`, `website/src/syntax.njk`, `website/src/syntax_ai.njk`, `website/src/differences.njk`, `website/src/cli.njk`, `website/src/embed.njk`, `website/src/playground.njk`, `website/src/blog/index.njk`, `website/src/blog/article1.njk`, `website/src/404.njk`, `website/src/_includes/layout.njk`, `website/src/assets/site.css`, `website/src/public/`
  ...
  - Canonical website-only syntax highlighter runtime shared by Eleventy and automated tests, with a TypeScript declaration contract for typed callers: `website/src/syntaxHighlight.mjs`, `website/src/syntaxHighlight.d.mts`, `website/src/syntaxHighlight.test.ts`
  - Esbuild-powered embeddable Monaco helper sources for single-file, tabbed, workspace, and full workbench tutorial editors; the build emits separate Monaco core and VexaScript language workers, while the preview-iframe document builder captures compile/runtime errors inside the embedded console: `website/scripts/buildEmbed.ts`, `website/src/assets/vexa-embed.ts`, `website/src/assets/monaco/vexaLanguageWorker.ts`, `website/src/assets/previewDocument.ts`
...
- GitHub automation:
  ...
  - Coverage job-summary renderer that aggregates LCOV line, function, and branch totals and links the tested commit to Codecov: `scripts/coverageSummary.ts` (tests: `scripts/coverageSummary.test.ts`)
  - Self-hosting CI driver that emits a runnable compiler with `tsc`, performs two byte-identical JavaScript generations, compares complete TypeScript/JavaScript-hosted modular C++ file sets plus two optimized native fixed-point generations, streams child stdout/stderr, records termination signals and the effective GCC/Clang optimization mode, times only C++ generation while compiling objects and linking separately, and publishes the seconds-based timing table to the GitHub Actions step summary: `scripts/selfHostCi.ts`
