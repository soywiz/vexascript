...
- CLI (`cli/`):
  ...
  - `cpp` and `cpp build` compile `.vx` and `.ts` entrypoints to a shared `program.hpp`, `main.cpp`, and deterministic `module-XXXX.cpp` files (or one translation unit with `--single-file`), while `cpp link` and `cpp run` reuse the same module graph, cache generated C++ and native executables under `<input>.build/`, and link with Oilpan. All native forms print their generation phase timings.
...
- Native C++ support:
  - Oilpan runtime, including reference-semantic traced `ArrayObject<T>` storage, a native `performance.now()` steady clock, a dependency-free arbitrary-precision integer implementation, UTF-16/UTF-8 operating-system boundary adapters, and the vendored standalone Oilpan (forked 20260622) and trimmed mimalloc source archives used by generated C++ builds: `native/runtime.cpp`, `native/bigint.h`, `native/utf.h`, `native/oilpan-20260622.zip`, `native/mimalloc-3.4.3.zip`
