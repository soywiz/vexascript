...
- CLI (`cli/`):
  ...
  - Node-only native build adapter that plans source-specific `<input>.build/main.cpp` intermediates, extracts the vendored Oilpan and mimalloc archives through CMake into `~/.vexascript/native`, creates platform- and architecture-named static library artifacts there, and links generated C++ with selectable `-O0`/`-O1`/`-O2`/`-O3`/`-Os`/`-Oz`/`-Og` levels and platform system libraries: `cli/nativeBuild.ts`
  ...
  - `cpp` and `cpp build` compile `.vx` and `.ts` entrypoints to C++, while `cpp link` and `cpp run` reuse the same module graph, cache generated C++ and native executables under `<input>.build/`, and link with Oilpan. All native forms print their generation phase timings.
