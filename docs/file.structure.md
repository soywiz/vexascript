...
- LSP server and features:
  ...
  - Shared, package-independent `.d.ts` declaration-graph primitives reused by both the node_modules typings loader and the ambient types loader so the two do not diverge (currently the canonical `/// <reference path>` extractor; see `docs/tasks/generalize-dts-declaration-loading.md` for the planned full general loader): `compiler/lsp/dtsModuleGraph.ts`
