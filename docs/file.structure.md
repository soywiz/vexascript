...
- Runnable samples and sample-test harness: `samples/`, `samples/samples.test.ts`
  ...
  - Syntax tour sample: `samples/syntax-tour/` is a multi-file runnable fixture that exercises the broad supported VexaScript syntax surface, JSX factory configuration, local imports, declarations/types, classes, extensions/operators, index operator overloads, property references, delegates, control flow, and async `sync` output.
...
- CLI (`cli/`):
  ...
  - `cpp` and `cpp build` compile `.vx` and `.ts` entrypoints to a shared `program.hpp`, `main.cpp`, and deterministic `module-XXXX.cpp` files, while `cpp link` and `cpp run` reuse the same module graph, cache generated C++ and native executables under `<input>.build/`, and link with Oilpan. All native forms print their generation phase timings.
