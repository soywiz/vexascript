...
- Runnable samples and sample-test harness: `samples/`, `samples/samples.test.ts`
  ...
  - Native language smoke sample: `samples/native-language-smoke/` is the multi-file end-to-end native fixture. Focused language semantics live under `language-features/`, and ECMAScript runtime families live under `standard-library/`. `expected.txt` validates JavaScript execution through the normal sample harness, while `tests/native/nativeSmoke.native.ts` invokes the public `cpp link` command, runs the linked Oilpan binary, and compares stdout with `expected.native.txt`.
