...
- Emitter / transpilation:
  ...
  - Browser-compatible C++ emission backend, sharing the normal parse and analysis pipeline; `compiler/runtime/nativeStandardLibraryCoverage.ts` locks every named ECMAScript runtime family to an executable case or an explicit unsupported diagnostic. Native behavior is validated by the compiled executable smoke rather than generated-code string assertions: `compiler/runtime/cppEmitter.ts`, smoke test: `tests/native/nativeSmoke.native.ts`, fixture: `samples/native-language-smoke/` (`standard-library.vx` aggregates the declaration-locked family modules under `standard-library/`), documented technical gaps: `docs/native-missing-runtime.md`.
