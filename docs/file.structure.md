...
- LSP server and features:
  ...
  - Bidirectional conditional rewrite quick fixes, including repeated-equality `if` chains to subject `match`, heterogeneous `if` chains to condition `match`, and safe `match` conversion back to `if`/`else if`/`else`: `compiler/lsp/matchConditionalFixes.ts`
