...
- GitHub automation:
  ...
  - Self-hosting CI driver that emits a runnable compiler with `tsc`, performs two byte-identical JavaScript generations, compares TypeScript/JavaScript-hosted C++ generation plus two native fixed-point generations, prepares native dependencies outside the measured compile/link intervals, and publishes the timing table to the GitHub Actions step summary: `scripts/selfHostCi.ts`
