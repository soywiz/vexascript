...
- LSP server and features:
  ...
  - Conditional rewrite quick fixes, including repeated-equality `if` chains to subject `match`, heterogeneous `if` chains to condition `match`, safe `match` conversion back to `if`/`else if`/`else`, and extraction of common per-arm `return` or stable-target assignment operations from exhaustive standalone matches: `compiler/lsp/matchConditionalFixes.ts`
