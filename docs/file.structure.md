...
- Semantic analysis:
  ...
  - Shared primitive matcher-name classification used by semantic analysis and JavaScript emission so `string`, numeric, boolean, and bigint-family patterns stay consistent: `compiler/analysis/matcherPatterns.ts`
...
- Emitter / transpilation:
  - Shared lowering pass boundary, including evaluation-order-preserving CPS lowering of VexaScript control-flow expressions (`if`, `return`, `throw`, `break`, and `continue`) before JavaScript emission: `compiler/runtime/lowering.ts`
  - Lowering tests: `compiler/runtime/lowering.test.ts`
  - JavaScript emission: `compiler/runtime/emitter.ts`
  - JavaScript FFI dynamic-library and ABI layout metadata: `compiler/runtime/foreignLibrary.ts`, `compiler/runtime/foreignStruct.ts`
  ...
  - Local module-graph bundling for execution and CLI ESM bundle preparation (resolves and inlines a `.vx` entry file together with its transitively imported local `.vx` and `.ts` modules so cross-file classes/operators/extension properties and TypeScript runtime declarations resolve before the CLI hands the emitted JavaScript to the Node-side package bundler). Its per-module type contexts propagate transitive project declarations across cycles while preserving project-local declaration priority over same-named dependency helpers, and its incremental cache stores the ordered declaration context used by analysis and emission: `compiler/runtime/moduleGraph.ts`
  - Shared explicit text-module import model (`import source from "./file.ext?text"`), including arbitrary-extension resolution and AST replacement with string constants: `compiler/runtime/textModuleImports.ts`
  - Shared node_modules import-resolution bridge used by the runtime bundler and single-file CLI build to reuse the richer declaration/type import pipeline without making core runtime modules depend directly on LSP-layer paths. It accepts the project's ambient declarations (including DOM types) and restricts package traversal to bare imports so local modules remain the responsibility of the module graph instead of being recollected once per module: `compiler/nodeModuleImportResolution.ts`
...
- Runnable samples and sample-test harness: `samples/`, `samples/samples.test.ts`
  ...
  - FFI sample convention: JavaScript foreign-library examples use a `samples/ffi-*` prefix so they remain distinct from normal JavaScript samples.
  - SDL2 FFI sample: `samples/ffi-sdl2/` opens a real window through Deno FFI with `@FFILibrary`, `@FFIStruct`, and `FFIPointer`. Its renderer passes a raw `ArrayBuffer` to `SDL_GetKeyboardState`, moves a rectangle with the arrow keys, polls overlapping `SDL_Event` field views, and yields each frame through nonblocking `SDL_Delay`.
- Formatter:
...
- CLI (`cli/`):
  ...
  - CLI entrypoint and command implementation, including single-file JavaScript transpilation, optional `build`/`bundle` input resolution through the configured `entrypoint`, type-checked `serve` with the explicit `--no-type-check` opt-out, and directory-based static site builds that materialize the `serve` bundle and mapped assets into `dist`/`outDir`: `cli/cli.ts`
...
- LSP server and features:
  ...
  - Shared synthetic invocation context for base-constructor arguments in `extends Base(...)`, reused by signature help, completion, hover, and definition: `compiler/lsp/baseConstructorInvocation.ts`
  ...
  - Conditional rewrite quick fixes, including repeated `==`/`===` equality chains to subject `match`, heterogeneous chains to condition `match`, consecutive fallthrough-terminating `if` statements treated as implicit chains, safe `match` conversion back to `if`/`else if`/`else`, and extraction of common per-arm `return` or stable-target assignment operations from exhaustive standalone matches: `compiler/lsp/matchConditionalFixes.ts`
...
- GitHub automation:
  - Continuous-integration workflow that runs the complete repository suite with LCOV coverage on Ubuntu, uploads same-repository coverage reports to Codecov, and performs JavaScript compiler self-hosting on an isolated runner on pushes to `main` and relevant pull requests: `.github/workflows/tests.yml`
  ...
  - Self-hosting CI driver that performs two byte-identical JavaScript generations and publishes the stage result to the GitHub Actions step summary: `scripts/selfHostCi.ts`
