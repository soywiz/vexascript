...
`const` is the canonical immutable spelling by default. `val` is an exact
alias and remains fully supported. Editor normalization quick fixes use the
...
- The return type is written **without** the `Promise<...>` wrapper. `sync func load(): Response` is internally an async function returning `Promise<Response>`; from the outside (and from other functions) the call is observed as `Promise<Response>`, so it participates in auto-await just like any other Promise.
...
VexaScript supports function declarations with four equivalent keywords:

- `func` (default canonical spelling)
- `fn`
- `fun`
- `function`
...
fun legacyAlias(a, b) => a + b
function typescriptStyle(a, b) => a + b
```

The editor offers normalization quick fixes on immutable and function
declaration keywords. Projects can select their canonical spellings in the
nearest `vexascript.json`:
...
`immutableDeclaration` accepts `"const"` or `"val"` and defaults to
`"const"`. `functionDeclaration` accepts `"func"`, `"fn"`, `"fun"`, or
`"function"` and defaults to `"func"`. These preferences control
normalization quick fixes and source generated by editor fixes; they do not
change what syntax the parser accepts.
...
`override` stub (`override func ...` for methods by default, `override ...` for properties)
for each missing abstract or interface member.

### Interfaces

VexaScript supports interface declarations, including generic parameters and `extends`. Interface members can also use explicit member keywords (`const` / `val` / `var` / `let` for properties and `func` / `fn` / `fun` / `function` for methods), while the older TypeScript-style member form still works:
...
The array element type is inferred from the result expression. Because `if` is
an expression, it may be used directly as the comprehension body. At the top
level of a comprehension body, an `if` without `else` acts as a filter: its
value is appended only when the condition is true, and false iterations append
nothing. The element type is inferred from the true branch alone. A
comprehension accepts one `for-in` or `for-of` header and one expression body;
classic three-clause loops, statement bodies, and `for await` are not supported
inside the brackets.
...
Comprehension elements accept the same separating and trailing commas as other
array elements, including when the array starts with a comprehension whose body
is an unbraced `if` expression.
