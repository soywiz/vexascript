...
* [~] Extract the general graph traversal, declaration parsing, re-export handling, and declaration-origin collection into a package-independent module. **In progress:** `compiler/lsp/dtsModuleGraph.ts` now owns the package-independent primitives — the `/// <reference path>` extractor (`extractTripleSlashReferencePaths`), source/program parsing with per-file mtime caches (`readDtsSource`/`parseDtsProgram`/`clearDtsModuleGraphCache`), and relative-specifier resolution (`resolveRelativeDtsPath`). Both `nodeModulesTypings.ts` and `ambientTypesLoader.ts` consume them; this collapsed two real divergences — the loaders previously used different `/// <reference path>` regexes (ambient missed single-quoted/leading-whitespace paths) and ambient parsed without any cache while node_modules cached. **Deliberately NOT moved:** `collectTypingsDeclarations`/`collectSelectiveTypingsDeclarations` + the entry/reexport model stay in the node_modules adapter, because the ambient loader produces a fundamentally different output (global vs per-module split), so there is no second consumer yet — moving the traversal now would be speculative relocation, not divergence collapse. Revisit if/when a second consumer needs the entry/reexport traversal (e.g. browser virtual workspace). The adapter seam is `resolveReexportedTypingsPath` (relative → `resolveRelativeDtsPath`, bare → `resolveNodeModulesTypingsPath`).
