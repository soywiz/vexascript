...
* [ ] Design one shared imported-symbol resolution record for local modules, ambient modules, and node_modules packages.
* [ ] Refactor imported-declaration collection so type info and declaration origin are produced by the same resolution pass where practical.
* [ ] Make hover and go to definition consume the shared imported-symbol result before falling back to feature-local heuristics.
...
* [ ] Imported bindings that have a resolved type also usually have a reusable declaration origin when the package declarations expose one.
* [ ] Zod-style namespace-shaped imports stop relying on one path for type information and another unrelated path for navigation.
* [ ] Hover and go to definition for the same imported binding agree on the same resolved declaration origin whenever one exists.
...
* [ ] Add focused regression tests where an imported binding previously had a type but no definition.
* [ ] Add coverage proving the same imported binding feeds analysis, hover, and definition consistently.
* [ ] Keep `samples/zod/` and `samples/pixi/` passing.
* [ ] Run `pnpm test`.
