...
Brace lambdas always carry a parser-synthesized `it` parameter, which initially made receiver lambdas conflate two different cases. For `T.(amount: int) -> void`, implicit `it` is the first visible argument (`amount`), while for `T.() -> void` there is no visible argument and the synthetic `it` may alias the receiver. Analysis and both emitters now record the latter case explicitly instead of treating every synthetic `it` as the receiver. The first loose lambda-analysis pass can still produce provisional symbol resolutions before contextual typing revisits the body; editor symbol lookup therefore prefers the latest resolution so hover and diagnostics expose the refined `int` type rather than stale `unknown` metadata.
...
Focused parser, type, analysis, JavaScript runtime, signature-help, and real Pixi LSP-session tests cover receiver syntax, first-argument compatibility, implicit `it` typing, shorthand blocks, nested labels, concrete-member precedence, inherited extensions, hover types, active call parameters, and Go to Definition into both project files and `node_modules`. The shared native language smoke exercises the same source constructs, implicit receiver aliases, and inherited-extension lowering through C++ compilation and execution.
