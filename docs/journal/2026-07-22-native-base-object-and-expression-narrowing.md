...
## Follow-up: shared static storage bases

Several concrete syntax and analysis classes repeated fields that are semantically common to a family of nodes. Because a pointer to the family base did not expose those fields, otherwise typed member access still required `dynamicGet`. `NamedNode` now owns the `name` field shared by named AST members, `CallableParameterNode` owns the parameter type/default fields, and `NamedAnalysisType` owns the analysis-type name. Concrete classes retain their existing constructor APIs while storing the fields on the common static base.

Together these bases reduce generated dynamic property reads from 482 to 401. The generated compiler is 6,315,429 bytes and contains 308 fewer `Value`, `toValue`, and `strictEquals` references. Its current `-O0` executable compiles in about 19.9 seconds and produces C++ in 63.27 seconds. The `-O1` executable takes 92.10 seconds to compile but emits the same C++ in 11.86 seconds, a 5.3x execution improvement over `-O0`. A second native roundtrip is byte-identical at both optimization levels.

An attempted general map for narrowing repeated member expressions such as `object.field instanceof Type && object.field.name` was rejected. It removed only six dynamic reads while adding roughly 29 KB of generated source and more intermediate `Value` operations. Shared class storage is the better direction because it makes the static fact part of the type model instead of adding emitter-local expression tracking.

## Follow-up: native compiler allocation and emission paths

The compiler's remaining anonymous analysis records were converted to ordinary classes with positional constructors, including scopes, flow contexts, function type parameters, symbol-resolution metadata, analysis symbols, and native class-field emission metadata. This keeps their generated native access static and removes duplicated record adapters. Native heap sizing is now adaptive, and the external TypeScript semantic check can overlap compiler work and reuse its result without changing the semantic command path.

String emission gained two focused allocation reductions. Flat, statically typed string concatenation chains use one pre-sized `concatText` operation instead of repeatedly allocating prefixes. The common `array.map(callback).join(separator)` form uses `mapJoin`, which evaluates the receiver and callback once, preserves callback order, and appends directly into the result without allocating the intermediate mapped array. The native language smoke executes concatenation with side effects so evaluation order cannot regress silently.

On the same semantic self-host workload, the warm `-O1` native compiler improves from roughly 4.34 seconds to 4.23-4.25 seconds. Node remains faster at roughly 3.72-3.77 seconds, so parity has not been reached. Current native phases are approximately 0.57 seconds for loading and parsing, 0.45 seconds for module isolation, 1.17-1.20 seconds for merged type inference, and 1.78 seconds for C++ emission. The current `-O0` compiler generates the complete compiler in 41.26 seconds. Compiling the generated compiler takes about 19.6 seconds at `-O0` and 91.2 seconds at `-O1`.

Several experiments were deliberately discarded. A failed native run that reported a much shorter emission phase terminated with `VexaScript value is not a string`; it was not a valid performance result and exposed a missing explicit native constructor-property annotation. Adding a `MapObject::tryGet` fast path and rvalue text conversions regressed the warm `-O1` result to about 4.39 seconds, so both were reverted. A conventional pool of `std::u16string` builders also cannot retain the useful allocation after returning by move: the result owns the buffer. Copying the capacity back into the pool would erase the gain. A durable pooled builder therefore requires emission into a caller-owned sink, or a dedicated pooled string representation, rather than wrapping the existing return-by-value emitter with another cache.

The latest full native-language smoke passes, and the native `-O0` and `-O1` compilers both generate the complete output successfully. Exact Node/native output equality remains pending: the files still differ in a small number of redundant nullish and pointer-conversion choices even though their behavior agrees.

## Follow-up: mutable typed string accumulation

Typed `string` compound assignments were accidentally routed through the dynamic addition machinery. A statement such as `text += fragment` boxed the current `std::u16string` as a `Value`, created a `StringObject` rope, immediately flattened that rope through `toText`, and replaced the native string. Repeating the operation therefore copied the growing prefix and lost both the rope and the native string's retained capacity.

When both operands are statically strings, C++ emission now uses native `std::u16string::operator+=`. The native string itself is the growable builder and retains capacity across iterations. Dynamic `Value` strings keep the rope path because their runtime type and JavaScript coercion rules still require it. The unified language smoke executes repeated mutable string appends in both JavaScript and native modes.

The self-hosted compiler drops from 77 typed-string `assignWith`/rope/flatten sequences to one genuinely non-static conditional case. Its `-O1` build takes 89.66 seconds, versus 90.10 seconds for the immediately preceding experiment. Warm semantic C++ generation improves from approximately 4.23-4.25 seconds to 4.02-4.07 seconds, with a 4.03-second median over four warm runs. A profiled run attributes most of the gain to loading and parsing, which falls from roughly 0.57 seconds to 0.42 seconds; merged type inference remains about 1.21 seconds and C++ emission about 1.77 seconds. Five native outputs are byte-identical to each other. Node/native byte identity still has the pre-existing small conversion differences.

## Follow-up: keyed static AST traversal

Native module isolation recursively enumerated every property of every AST node with `for...in`. In generated C++, this became dynamic key enumeration followed by dynamic property reads and array checks, even though `childNodes` already held the complete statically typed structural traversal table. The traversal table now optionally appends the corresponding field key alongside each child. Native identifier resolution consumes that same source of truth with reusable typed child/key buffers and iterative typed stacks, preserving scope entry/exit order without dynamic AST property access.

An experiment that only reused the temporary `childNodes` array did not improve the benchmark: warm native results stayed around 4.21-4.39 seconds, so that change was discarded. Removing the dynamic enumeration is materially different. Native module isolation falls from roughly 460 milliseconds to 98 milliseconds. The `-O1` compiler builds in 89.48 seconds and completes warm semantic C++ generation in 3.73, 3.73, 3.90, 3.86, and 3.79 seconds, a 3.79-second median. The same Node compiler version completes in 3.67, 3.67, 3.72, 3.70, and 3.67 seconds, a 3.67-second median. The native gap is therefore about 0.12 seconds on this workload; parity is close but not yet achieved.
